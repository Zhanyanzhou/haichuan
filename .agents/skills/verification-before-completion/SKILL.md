...
涉及实现、集成或发布声明时，按项目 `WORKFLOW.md` 区分本地实现、真实联调和上线层级。高一层未请求、未授权或环境不可达，不自动否定已有证据支持的低一层结果；低一层证据也不能被外推为高一层完成。本地完成条件中的关键路径尚未验证时，必须降级为“已完成代码修改，待本地验收”。纯解释和只读审查只核对其结论对应的证据。
...
按 `WORKFLOW.md` 第 6 节选择与任务类型对应的交付内容，说明实际命令或检查、退出结果、环境、覆盖范围和未验证项。实现类结论不得把未请求的更高层验证表述成本地实现遗漏；存在真实上线阻断时再明确标为阻断。纯解释和只读审查不要求实现状态栏目。大日志只摘要关键证据并保留可定位信息，不把整段原始输出反复带入会话。
