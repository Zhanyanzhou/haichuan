...
7) Validate material assumptions and service context
   - Infer context from repository evidence where possible and summarize assumptions that materially affect scope or risk ranking.
   - Ask 1 to 3 targeted questions and pause only when the missing context would materially change scope or priority and cannot be resolved from the repository.
   - Otherwise proceed with explicit assumptions and mark conditional conclusions; do not manufacture a mandatory check-in for well-evidenced cases.
...
When unresolved context would materially change scope or priority, first provide an assumption-validation check-in:
- List the key assumptions in 3 to 6 bullets.
- Ask 1 to 3 targeted context questions and wait for the response.

When repository evidence is sufficient, omit that check-in and produce the report directly while retaining explicit assumptions and open questions.
