...
export default function TemplateEditorToolbar({
  ...
  const recovery = draft?.compatibilityRecovery;
  const toolbarStateMode = recovery?.status === "source-invalid"
    || saveFailed
    || permissionDenied
    || publishFailed
      ? "error"
      : conflicted || recovery
        ? "warning"
        : dirty || needsInitialSave || hasUnpublishedDraftChanges
          ? "dirty"
          : busy
            ? "pending"
            : "clean";
  const toolbarStateLabel = recovery?.status === "source-invalid"
    ? "原草稿需处理"
    : recovery
      ? "修复方案未保存"
      : lifecycleBusy
                      ...
                      : needsInitialSave
                        ...
                        : hasUnpublishedDraftChanges
                          ...
                          : draft
                            ...
                            : "未选择模板";
  const toolbarStateAriaLabel = recovery?.status === "source-invalid"
    ? "模板状态：原草稿需处理"
    : recovery
      ? "模板状态：修复方案未保存"
      : lifecycleBusy
                        ...
                        : hasUnpublishedDraftChanges
                          ...
                          : draft
                            ? (localOnly
                              ...
                              : "模板状态：模板草稿已保存")
                            : "模板状态：未选择模板";
  const toolbarStateAction = recovery ? onOpenRecovery : publishReview ? onOpenPublishReview : undefined;
  ...
  const toolbar = (
    <header className="homepage-editor__toolbar template-editor__toolbar">
      ...
      <WorkspaceToolbarActions
        ...
        save={{
          label: conflicted ? "另存副本" : saveFailed || permissionDenied ? "重试保存" : localOnly ? "保存本机草稿" : "保存草稿",
          loading: saving,
        ...
        publish={{
          label: localOnly && !onPublish ? "不可发布" : publishFailed ? "重试发布" : "用于页面",
          loading: publishing,
