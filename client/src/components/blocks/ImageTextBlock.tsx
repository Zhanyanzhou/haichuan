...
export default function ImageTextBlock({ module }: ImageTextBlockProps) {
    ...
    return (
      <section style={{ padding: paddingMap[spacing], background: '#FBF9F6' }}>
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 24px', textAlign: 'center' }}>
          {label && (
            <p style={{ fontSize: 11, letterSpacing: '0.3em', textTransform: 'uppercase', color: '#B8944E', marginBottom: 16 }}>
              {label}
            </p>
          )}
          {title && (
            <h2 style={{ fontSize: 36, fontFamily: '"Cormorant Garamond","Noto Serif SC",serif', color: '#2C2C2C', marginBottom: 24, lineHeight: 1.3 }}>
              {title}
            </h2>
          )}
          {body && (
            <p style={{ fontSize: 15, color: '#8A7F72', lineHeight: 1.8, maxWidth: 560, margin: '0 auto' }}>
              {body}
            </p>
          )}
          {buttonText && linkUrl && (
  ...
  if (template === 'imageBackground') {
    return (
      <section style={{ position: 'relative', minHeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: '#2C2C2C' }}>
        {image && (
          <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${image})`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.5 }} />
        )}
        <div style={{ position: 'relative', zIndex: 1, padding: 60, textAlign: 'center', maxWidth: 720 }}>
          {label && <p style={{ fontSize: 11, letterSpacing: '0.3em', color: '#B8944E', marginBottom: 12 }}>{label}</p>}
          {title && <h2 style={{ fontSize: 40, fontFamily: '"Cormorant Garamond","Noto Serif SC",serif', color: '#fff', marginBottom: 20 }}>{title}</h2>}
          {body && <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.8)', lineHeight: 1.7 }}>{body}</p>}
          {buttonText && linkUrl && (
            <Link to={linkUrl} style={{ display: 'inline-block', marginTop: 24, padding: '10px 40px', border: '1px solid #B8944E', color: '#B8944E', fontSize: 13, textDecoration: 'none' }}>
              {buttonText}
            </Link>
          )}
        </div>
  ...
  const imageCol = image ? (
    <div className="homepage-image-text__image" style={{ flex: isImageLeft ? '0 0 55%' : '0 0 45%' }}>
  ...
  const textCol = (
    <div className="homepage-image-text__copy" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 40px' }}>
      <div style={{ maxWidth: 440 }}>
        {label && <p style={{ fontSize: 11, letterSpacing: '0.3em', textTransform: 'uppercase', color: '#B8944E', marginBottom: 16 }}>{label}</p>}
        {title && <h2 style={{ fontSize: 32, fontFamily: '"Cormorant Garamond","Noto Serif SC",serif', color: '#2C2C2C', marginBottom: 18, lineHeight: 1.25 }}>{title}</h2>}
        {body && <p style={{ fontSize: 14, color: '#8A7F72', lineHeight: 1.8 }}>{body}</p>}
        {buttonText && linkUrl && (
          <Link to={linkUrl} style={{ display: 'inline-block', marginTop: 20, padding: '8px 32px', border: '1px solid #B8944E', color: '#B8944E', fontSize: 12, textDecoration: 'none', letterSpacing: '0.1em' }}>
            {buttonText}
          </Link>
        )}
      </div>
