...
export default function HeroSection({ module, editMode }: Props) {
  ...
  } as CSSProperties & Record<'--hc-hero-focus-desktop' | '--hc-hero-focus-mobile', string>;
  ...
  return (
    ...
    >
      ...
      {imageFailed ? (
        <div className="absolute inset-0 grid place-items-center text-xs tracking-[.08em]" role="img" aria-label={c?.altText || "主视觉图片暂不可用"} style={{ color: '#7E7468', background: 'linear-gradient(135deg,#EDE6DC,#D9CDBD)' }}>
          主视觉图片暂不可用
        </div>
      ) : null}
      ...
      >
        ...
        {actionText && targetUrl ? (
          editMode ? (
            <span data-editor-field="actionText linkUrl productId" className="inline-flex items-center gap-2 text-[10px] md:text-[11px] tracking-[.14em] uppercase"
              style={{ color: 'rgba(255,255,255,0.7)', fontFamily: 'Inter,system-ui,sans-serif' }}>
              {actionText} <span>→</span>
            </span>
          ) : (
            <Link data-editor-field="actionText linkUrl productId" to={targetUrl} className="inline-flex items-center gap-2 text-[10px] md:text-[11px] tracking-[.14em] uppercase transition-opacity duration-300 hover:opacity-60"
