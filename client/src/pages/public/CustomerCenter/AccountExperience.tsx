...
type Section = "overview" | "selections" | "appointments" | "orders" | "favorites" | "profile";
...
const navItems: Array<{ key: Section; number: string; label: string; detail: string }> = [
  { key: "overview", number: "01", label: "账户首页", detail: "专属服务概览" },
  { key: "selections", number: "02", label: "我的选款", detail: "心仪作品与进度" },
  { key: "appointments", number: "03", label: "我的预约", detail: "顾问与到店安排" },
...
function MemberAccess({ authLoading, onLogin, onRegister }: Pick<AccountExperienceProps, "authLoading" | "onLogin" | "onRegister">) {
  ...
  return (
    <form className="account-member-access" onSubmit={(event) => {
      ...
      <div className="account-access-tabs">
        <button type="button" className={mode === "login" ? "is-active" : ""} onClick={() => setMode("login")}>会员登录</button>
        <button type="button" className={mode === "register" ? "is-active" : ""} onClick={() => setMode("register")}>注册会员</button>
      </div>
      <p>{mode === "login" ? "登录后，提交咨询无需重复填写联系方式。" : "注册后，为您保存作品偏好、咨询与订单档案。"}</p>
      {mode === "register" && <label>称呼<input value={name} onChange={(event) => setName(event.target.value)} maxLength={50} required /></label>}
      <label>手机号<input value={phone} onChange={(event) => setPhone(event.target.value)} inputMode="numeric" maxLength={11} required /></label>
      {mode === "register" && <label>邮箱（选填）<input value={email} onChange={(event) => setEmail(event.target.value)} type="email" maxLength={100} /></label>}
      ...
      <button type="submit" className="account-button account-button--dark" disabled={authLoading}>{authLoading ? "处理中…" : mode === "login" ? "登录我的账户" : "创建会员账户"}</button>
    </form>
  );
}
...
export default function AccountExperience({
  ...
  const stats = useMemo(
    () => [
      { value: String(selectionInquiries.length).padStart(2, "0"), label: "选款咨询" },
    ...
    return (
      <main className="account-page account-page--guest">
        ...
        <section className="account-hero">
          ...
          <h1>您的珠宝档案，<br />值得被悉心珍藏。</h1>
          <p className="account-hero__text">登录后，作品偏好、专属咨询、预约与订单将被妥善保存于同一处。</p>
          <div className="account-hero__actions">
            <button type="button" className="account-button account-button--dark" disabled title="会员账号服务正在接入">
              会员登录即将开放
            </button>
            <Link to="/catalog" className="account-text-link">先浏览作品 <span>→</span></Link>
          </div>
        </section>
        <section className="account-guest-grid">
          <div className="account-benefits">
            <p className="account-kicker">MEMBER EXPERIENCE</p>
            <h2>为每一次心动，<br />留下恰到好处的记录。</h2>
            <div className="account-benefit-list">
              <div><span>01</span><p><strong>专属选款</strong>保存心仪作品，顾问了解您的偏好。</p></div>
              <div><span>02</span><p><strong>一对一预约</strong>集中管理线上咨询与到店安排。</p></div>
              <div><span>03</span><p><strong>订单档案</strong>随时查看订单、支付与售后服务。</p></div>
            </div>
          </div>
  ...
  const renderSection = () => {
      ...
      return (
        <div className="account-overview">
          <section className="account-welcome">
            ...
            <div className="account-stats">{stats.map((stat) => <div key={stat.label}><strong>{stat.value}</strong><span>{stat.label}</span></div>)}</div>
          </section>
          <div className="account-content-grid">
            <section className="account-panel">
              <div className="account-panel__head"><div><p className="account-kicker">RECENT ORDER</p><h2>最近订单</h2></div><button type="button" onClick={() => setSection("orders")}>查看全部</button></div>
              {firstOrder ? <div className="account-order"><div><small>{firstOrder.orderNo}</small><h3>{firstOrder.items?.[0]?.product?.name || "珠宝作品"}</h3><p>{new Date(firstOrder.createdAt).toLocaleDateString("zh-CN")} · {statusLabels[firstOrder.status] || firstOrder.status}</p></div><strong>¥{Number(firstOrder.finalAmount).toLocaleString("zh-CN")}</strong></div> : <EmptyState title="尚未有订单记录" description="探索心仪作品，开始您的珠宝故事。" action={<Link to="/catalog" className="account-text-link">进入选款中心 →</Link>} />}
            </section>
            <section className="account-panel account-panel--gold">
              <p className="account-kicker">PERSONAL SERVICE</p><h2>预约专属顾问</h2><p>从挑选到佩戴，为您安排一对一珠宝顾问服务。</p><Link to="/contact" className="account-button account-button--light">预约咨询</Link>
            </section>
          </div>
        </div>
      );
    ...
    if (section === "orders") {
      return <section className="account-panel"><div className="account-panel__head"><div><p className="account-kicker">ORDERS</p><h2>我的订单</h2></div></div>{orders.length ? <div className="account-order-list">{orders.map((order) => <div className="account-order" key={order.id}><div><small>{order.orderNo}</small><h3>{order.items?.[0]?.product?.name || "珠宝作品"}</h3><p>{new Date(order.createdAt).toLocaleDateString("zh-CN")} · {statusLabels[order.status] || order.status}</p></div><strong>¥{Number(order.finalAmount).toLocaleString("zh-CN")}</strong></div>)}</div> : <EmptyState title="尚未有订单记录" description="浏览作品，开始您的专属珠宝档案。" action={<Link to="/catalog" className="account-text-link">进入选款中心 →</Link>} />}</section>;
    }
    if (section === "profile") {
      return <section className="account-panel"><div className="account-panel__head"><div><p className="account-kicker">PROFILE</p><h2>个人资料</h2></div></div><div className="account-profile-grid"><div><span>姓名</span><strong>{displayName}</strong></div><div><span>手机号</span><strong>{profile?.phone || "—"}</strong></div><div><span>邮箱</span><strong>{profile?.email || "暂未填写"}</strong></div></div><div className="account-address"><p className="account-kicker">DELIVERY ADDRESS</p><h3>常用收货地址</h3>{addresses.length ? addresses.map((address) => <p key={address.id}>{address.recipientName} · {address.recipientPhone}<br />{[address.province, address.city, address.district, address.detail].filter(Boolean).join("")}</p>) : <p className="account-muted">暂未保存收货地址</p>}</div></section>;
    }
    if (section === "selections" || section === "appointments") {
      ...
      const emptyDescription = section === "selections" ? "心仪的作品会在这里等待您" : "预约咨询与到店安排会在这里呈现";
      const path = section === "selections" ? "/catalog" : "/contact";
      return <section className="account-panel"><div className="account-panel__head"><div><p className="account-kicker">PRIVATE SERVICE</p><h2>{heading}</h2></div></div>{records.length ? <div className="account-order-list">{records.map((record) => <div className="account-order" key={record.id}><div><small>{new Date(record.createdAt).toLocaleDateString("zh-CN")}</small><h3>{section === "selections" ? (record as typeof selectionInquiries[number]).items?.[0]?.productNameSnapshot || "选款咨询" : (record as typeof inquiries[number]).product?.name || (record as typeof inquiries[number]).consultationType || "预约咨询"}</h3><p>{record.status === "PENDING" ? "待顾问联系" : record.status === "PROCESSING" ? "顾问跟进中" : record.status === "REPLIED" ? "已回复" : record.status}</p></div><strong>→</strong></div>)}</div> : <EmptyState title={heading} description={emptyDescription} action={<Link to={path} className="account-button account-button--outline">{section === "selections" ? "去选款中心" : "预约咨询"}</Link>} />}</section>;
    }
    const labels: Record<Exclude<Section, "overview" | "orders" | "profile" | "selections" | "appointments">, [string, string, string]> = { favorites: ["我的收藏", "珍藏每一次令人心动的相遇", "浏览作品"] };
    const [title, description, action] = labels[section as keyof typeof labels];
    return <section className="account-panel"><p className="account-kicker">PRIVATE COLLECTION</p><EmptyState title={title} description={description} action={<Link to="/catalog" className="account-button account-button--outline">{action}</Link>} /></section>;
  };

  return (
    <main className="account-page">
      <style>{accountStyles}</style>
      <section className="account-header"><div><p className="account-kicker">HAICHUAN JEWELRY</p><h1>我的账户</h1></div><button type="button" className="account-signout" onClick={onSignOut}>退出登录</button></section>
      <div className="account-layout">
        <aside className="account-nav" aria-label="账户导航">{navItems.map((item) => <button type="button" className={section === item.key ? "is-active" : ""} onClick={() => setSection(item.key)} key={item.key}><span>{item.number}</span><div><strong>{item.label}</strong><small>{item.detail}</small></div></button>)}</aside>
...
const accountStyles = `
.account-hero__actions > button{display:none}
