...
function getSuggestions(
  ...
  walk(categories);
  products
    .filter((p) => p.sku.toLowerCase().includes(q.toLowerCase()))
    .slice(0, 2)
    .forEach((p) => add("货号", p.sku));
...
function FilterDrawer({
  ...
  filters: Filters;
  onFilter: (key: keyof Filters, v: string) => void;
  onClear: () => void;
  onClose: () => void;
  count: number;
  categories: RealCategory[];
    ...
    <>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          <FGroup
            title="材质"
            options={MATERIALS.map((m) => ({ id: m, name: m }))}
        ...
        {options.map((o) => (
          <label
            ...
            style={{
              ...
              cursor: "pointer",
              fontSize: 13,
              color: selected === o.id ? T.txt : T.sec,
              paddingBlock: 2,
            }}
          >
            <input
...
function FilterTags({
  ...
  if (filters.category)
    tags.push({
      key: "category",
      label:
        findCat(categories, Number(filters.category)) ||
  ...
  return (
    ...
    >
      ...
      {tags.map((t) => (
        ...
        >
          {t.label}
          <span style={{ color: T.light, marginInline: 2 }}>×</span>
        </span>
      ))}
      <button
        onClick={onClearAll}
...
function ProductCard({ product }: { product: CatalogProduct }) {
  ...
  return (
    ...
    >
      <Link
        to={`/catalog?category=${product.primaryCategoryId}&query=${product.sku}`}
      ...
      >
        ...
        <p style={{ fontSize: 12, color: T.sec, margin: "0 0 2px" }}>
          {product.series}
        </p>
    ...
    <div style={{ background: T.bg, minHeight: "100vh" }}>
      ...
      {isShowingResults ? (
        <section style={{ paddingBottom: 120 }}>
          <div
            style={{ maxWidth: MW, marginInline: "auto", paddingInline: PX }}
          >
            {/* 搜索说明 */}
            {hasQuery && (
            ...
            {sorted.length === 0 ? (
              <div style={{ textAlign: "center", paddingBlock: 72 }}>
                ...
                <p style={{ fontSize: 14, color: T.sec, marginBottom: 28 }}>
                  请检查关键词或减少筛选条件
                </p>
                <div
              ...
              <div
                ...
                style={{
                  display: "grid",
                  gridTemplateColumns: "repeat(2,1fr)",
                  columnGap: "clamp(22px,2.5vw,32px)",
                  rowGap: "clamp(44px,5.5vh,64px)",
        ...
        <section
          style={{
            borderTop: `1px solid ${T.line}`,
            paddingBlock: "clamp(36px,5vh,60px)",
          }}
        >
          <div
            style={{ maxWidth: MW, marginInline: "auto", paddingInline: PX }}
              ...
              >
                {HOT.map((tag) => (
                  <button
                    ...
                    onMouseEnter={(e) => {
                      e.currentTarget.style.borderColor = T.txt;
                      e.currentTarget.style.color = T.txt;
                    }}
                    onMouseLeave={(e) => {
