...
const SECTIONS = [
  ...
  {
    title: "行为分析",
    body: "行为分析功能默认关闭。只有在该功能已启用且您明确同意后，本站才会采集页面访问、点击等浏览事件，并使用仅在当前浏览器会话中有效的随机标识区分访问次数。该标识不能单独识别您的身份，也不会与姓名、手机号等资料关联用于定向营销；拒绝或撤回同意后不再发送新的分析事件。",
  },
  {
    title: "信息保存",
    items: [
  ...
  {
    title: "本地存储",
    body: "登录会话由浏览器以 HttpOnly Cookie 管理，页面脚本不能读取其中的会话凭证。浏览器本地存储用于保存尚未提交的选款清单；同意分析后，随机分析标识仅保存在当前会话存储中，您的同意选择保存在 Cookie 中。您可以在浏览器设置中清除这些数据。",
  },
  {
    title: "未成年人",
