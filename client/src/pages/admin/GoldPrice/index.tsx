...
export default function GoldPrice() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="!bg-white !border-brand-line" title={<span className="font-semibold text-brand-text">金价历史</span>} loading={loading}>
        <Table dataSource={history} rowKey="id" pagination={false} size="middle"
          columns={[
            ...
            { title: '涨跌', dataIndex: 'change', width: 100, render: (v: number) => <span style={{ color: v >= 0 ? 'var(--adm-success)' : 'var(--adm-error)' }}>{v >= 0 ? <ArrowUpOutlined className="mr-1" /> : '↓'}{Math.abs(v || 0).toFixed(2)}</span> },
