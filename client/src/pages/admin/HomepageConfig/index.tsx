    ...
    case "首屏主视觉":
      return [
        ...
        },
        {
          ...
          ...createCropPreview("9 / 16", props.focusX, props.focusY),
        },
      ];
    case "单图海报":
      return [
      ...
      return [
        {
          ...
          ...createCropPreview("4 / 3", props.mainFocusX, props.mainFocusY),
        },
        {
          field: "detailImage",
          label: "细节海报",
      ...
      return [
        {
          ...
          ...createCropPreview("12 / 5", props.focusX, props.focusY),
        },
        {
          field: "mobileImage",
          label: "移动端背景图",
          ...
          ...createCropPreview("5 / 6", props.focusX, props.focusY),
        },
      ];
    case "分割面板":
      return [
...
function HeroInspector({
  ...
  const imageUrl =
    configuredImageUrl || (device === "mobile" ? props.desktopImage || "" : "");
  const isFallback =
    device === "mobile" && !configuredImageUrl && Boolean(props.desktopImage);
  ...
  const focusX = Math.min(
    100,
    Math.max(0, Number(props[focusXField] ?? props.focusX ?? 50)),
  );
  const focusY = Math.min(
    100,
    Math.max(0, Number(props[focusYField] ?? props.focusY ?? 50)),
  );
  const spec =
    device === "mobile" ? IMAGE_SPECS.hero.mobile : IMAGE_SPECS.hero.desktop;
  ...
  const format =
    meta.format ||
    (imageUrl.match(/\.(webp|avif|jpe?g|png|gif)/i)?.[1] || "").toLowerCase();
    ...
    >
      <header className="homepage-editor__inspector-header">
        <strong className="homepage-editor__inspector-title">
          {getModuleDisplayName("首屏主视觉", props)}
        </strong>
        <span className="homepage-editor__inspector-device">
          {device === "mobile" ? "移动端" : "桌面端"}
        </span>
        ...
        >
          {isFallback ? (
            <div className="homepage-editor__media-status is-fallback">
              <ExclamationCircleOutlined />
              <div>
                <strong>当前复用桌面端主视觉</strong>
                <span>建议上传9:16竖图，并为移动端单独设置焦点。</span>
              </div>
            </div>
          ) : null}
          {hasImage ? (
            <>
              ...
              <details className="homepage-editor__inspector-details">
                ...
                <FocusPicker
                  ...
                  safeArea={safeArea}
                  onChange={(x, y) =>
                    update({ [focusXField]: x, [focusYField]: y })
                  }
                />
        ...
        <InspectorSection title="文字内容" resetKey={props.id}>
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.title || "").length}/
                {HERO_CONTRACT.content.limits.title}
              </span>
            </label>
            <Input
              value={props.title || ""}
              ...
              placeholder="主标题"
              status={props.title?.trim() ? undefined : "error"}
            />
          </div>
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                ...
                {HERO_CONTRACT.content.limits.subtitle}
              </span>
            </label>
            <Input.TextArea
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              按钮文字
              <span className="homepage-editor__inspector-count">
                {(props.actionText || "").length}/
                {HERO_CONTRACT.content.limits.actionText}
              </span>
            </label>
            <Input
        ...
        >
          <div className="homepage-editor__inspector-field">
            ...
            <Input
              ...
              maxLength={HERO_CONTRACT.content.limits.altText}
              placeholder="描述这张主视觉图"
            />
          </div>
        </InspectorSection>
      </div>
...
function ImageTextInspector({
  ...
  return (
    ...
    >
      <header className="homepage-editor__inspector-header">
        <strong className="homepage-editor__inspector-title">
          {getModuleDisplayName("图文混排", props)}
        </strong>
        <span className="homepage-editor__inspector-device">
          {device === "mobile" ? "移动端" : "桌面端"}
        </span>
      ...
      <div className="homepage-editor__inspector-scroll">
        <p className="homepage-editor__properties-helper">
          {IMAGE_TEXT_CONTRACT.purpose}
        </p>
        <div
          className={`homepage-editor__contract-status is-${statusTone}`}
          role="status"
        >
          {statusTone === "ready" ? (
            <CheckCircleOutlined />
        ...
        <InspectorSection title="基础内容" resetKey={props.id}>
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.label || "").length}/
                {IMAGE_TEXT_CONTRACT.content.limits.label}
              </span>
            </label>
            <Input
              value={props.label || ""}
              onChange={(event) => update({ label: event.target.value })}
              maxLength={IMAGE_TEXT_CONTRACT.content.limits.label}
              placeholder="例如 BRAND STORY"
            />
          </div>
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.title || "").length}/
                {IMAGE_TEXT_CONTRACT.content.limits.title}
              </span>
            </label>
            <Input
              value={props.title || ""}
              onChange={(event) => update({ title: event.target.value })}
              maxLength={IMAGE_TEXT_CONTRACT.content.limits.title}
              placeholder="品牌故事"
              status={props.title?.trim() ? undefined : "error"}
            />
          </div>
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.body || "").length}/
                {IMAGE_TEXT_CONTRACT.content.limits.body}
              </span>
            </label>
            <Input.TextArea
              value={props.body || ""}
              onChange={(event) => update({ body: event.target.value })}
              maxLength={IMAGE_TEXT_CONTRACT.content.limits.body}
              rows={4}
              placeholder="用 2—4 行说明设计理念、材质或品牌故事"
            />
          </div>
          <div className="homepage-editor__inspector-field">
            <label>
            ...
            <Input
              ...
              maxLength={IMAGE_TEXT_CONTRACT.content.limits.buttonText}
              placeholder="查看详情"
            />
          </div>
        </InspectorSection>
        ...
        {needsImage && (
          <InspectorSection title="图片素材" resetKey={props.id}>
            <MediaPickerField
              ...
              placeholder="上传图文配图"
              previewAspectRatio={
                IMAGE_TEXT_CONTRACT.canvas.desktopMediaAspectRatio
              }
              previewFocus={{ x: focusX, y: focusY }}
            />
          </InspectorSection>
        ...
        <InspectorSection title="布局" defaultOpen={false} resetKey={props.id}>
          <div className="homepage-editor__inspector-option-group">
            ...
            <div
              className="homepage-editor__inspector-segmented is-grid"
              role="group"
              aria-label="图文展示方式"
            >
              {[
                ...
                <button
                  key={value}
                  type="button"
                  className={template === value ? "is-active" : ""}
          ...
          <div className="homepage-editor__inspector-option-group">
            ...
            <div
              className="homepage-editor__inspector-segmented is-three"
              role="group"
              aria-label="内容留白"
            >
              {[
                ...
                <button
                  ...
                  type="button"
                  className={
                    (props.spacing || "normal") === value ? "is-active" : ""
                  }
                  aria-pressed={(props.spacing || "normal") === value}
          ...
          >
            <div className="homepage-editor__inspector-field">
              ...
              <Input
                ...
                maxLength={IMAGE_TEXT_CONTRACT.content.limits.imageAlt}
                placeholder="描述图片中的人物、珠宝或场景"
              />
            </div>
            {props.image && (
              <div className="homepage-editor__inspector-field">
...
function SinglePosterInspector({
  ...
  return (
    ...
    >
      <InspectorHeader
        title={getModuleDisplayName("单图海报", props)}
        device={device}
        onClose={close}
      />
      <div className="homepage-editor__inspector-scroll">
        <p className="homepage-editor__properties-helper">
          {SINGLE_POSTER_CONTRACT.purpose}
        </p>
        <ContractStatusBanner status={status} />
        ...
        <InspectorSection title="基础内容" resetKey={props.id}>
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.number || "").length}/
                {SINGLE_POSTER_CONTRACT.content.limits.number}
              </span>
            </label>
            <Input
              value={props.number || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.label || "").length}/
                {SINGLE_POSTER_CONTRACT.content.limits.label}
              </span>
            </label>
            <Input
              value={props.label || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.title || "").length}/
                {SINGLE_POSTER_CONTRACT.content.limits.title}
              </span>
            </label>
            <Input
              value={props.title || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.subtitle || "").length}/
                {SINGLE_POSTER_CONTRACT.content.limits.subtitle}
              </span>
            </label>
            <Input
              value={props.subtitle || ""}
          ...
          <div className="homepage-editor__inspector-field">
            ...
            <Input
              ...
              status={props.linkUrl?.trim() ? undefined : "error"}
            />
          </div>
        </InspectorSection>
        ...
        >
          {device === "mobile" && !props.mobileImage ? (
            <div className="homepage-editor__media-status is-fallback">
              <ExclamationCircleOutlined />
              <div>
                <strong>当前复用桌面端海报</strong>
                <span>建议上传3:4竖图，避免主体被自动裁切。</span>
              </div>
            </div>
          ) : null}
          <MediaPickerField
            ...
            spec={imageSpec}
            placeholder={
              device === "mobile" ? "上传移动端竖版海报" : "上传桌面端海报"
            }
            previewAspectRatio={aspectRatio}
            previewFocus={{ x: focusX, y: focusY }}
          />
        ...
        <InspectorSection title="布局" defaultOpen={false} resetKey={props.id}>
          <div className="homepage-editor__inspector-option-group">
            ...
            >
              {[
                ["leftTextRightImage", "文左图右"],
                ["leftImageRightText", "图左文右"],
              ].map(([value, label]) => (
                <button
                  key={value}
                  type="button"
                  className={
                    (props.template || "leftTextRightImage") === value
                      ? "is-active"
                      : ""
                  }
                  aria-pressed={
          ...
          <div className="homepage-editor__layout-rule">
            <strong>画布标准</strong>
            <span>桌面：文案1/4、图片3/4，图片固定3:2</span>
            <span>移动：图片3:4，文案置于图片下方</span>
          </div>
        </InspectorSection>
        ...
        {(props.desktopImage || props.mobileImage) && (
          ...
          >
            <div className="homepage-editor__inspector-field">
              ...
              <FocusPicker
                src={
                  props[imageField] || props.desktopImage || props.mobileImage
                }
                focusX={focusX}
                focusY={focusY}
                aspectRatio={aspectRatio}
...
function FullBleedInspector({
  ...
  const previewImage =
    props[imageField] || props.image || props.mobileImage || "";
  ...
  return (
    ...
    >
      <InspectorHeader
        title={getModuleDisplayName("全屏出血图", props)}
        device={device}
        onClose={close}
      />
      <div className="homepage-editor__inspector-scroll">
        <p className="homepage-editor__properties-helper">
          {FULL_BLEED_CONTRACT.purpose}
        </p>
        <ContractStatusBanner status={status} />
        ...
        >
          {device === "mobile" && !props.mobileImage && props.image ? (
            <div className="homepage-editor__media-status is-fallback">
              ...
              <div>
                <strong>当前复用桌面端海报</strong>
                <span>建议上传5:6竖图，避免人物或珠宝主体被裁切。</span>
              </div>
            </div>
          ) : null}
          <MediaPickerField
            ...
            spec={imageSpec}
            placeholder={
              device === "mobile" ? "上传移动端5:6海报" : "上传桌面端12:5海报"
            }
            previewAspectRatio={aspectRatio}
            previewFocus={{ x: focusX, y: focusY }}
          />
        ...
        <InspectorSection title="文字内容" resetKey={props.id}>
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.title || "").length}/
                {FULL_BLEED_CONTRACT.content.limits.title}
              </span>
            </label>
            <Input
              value={props.title || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.subtitle || "").length}/
                {FULL_BLEED_CONTRACT.content.limits.subtitle}
              </span>
            </label>
            <Input.TextArea
              value={props.subtitle || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              引导文字
              <span className="homepage-editor__inspector-count">
                {(props.buttonText || "").length}/
                {FULL_BLEED_CONTRACT.content.limits.actionText}
              </span>
            </label>
            <Input
              value={props.buttonText || ""}
        ...
        >
          <div className="homepage-editor__inspector-option-group">
            <div>
              <strong>文字位置</strong>
              <span>只提供稳定版式，避免自由拖拽破坏海报构图。</span>
            </div>
            <div
              className="homepage-editor__inspector-segmented is-grid"
              role="group"
            ...
            >
              {[
                ...
                <button
                  ...
                  type="button"
                  className={
                    (props.template || "textCenter") === value
                      ? "is-active"
                      : ""
                  }
                  aria-pressed={(props.template || "textCenter") === value}
                  onClick={() => update({ template: value })}
                >
                  {label}
                </button>
            ...
            >
              {[
                ...
                <button
                  ...
                  type="button"
                  className={
                    (props.overlayPreset || "soft") === value ? "is-active" : ""
                  }
                  aria-pressed={(props.overlayPreset || "soft") === value}
                  onClick={() => update({ overlayPreset: value })}
                >
                  {label}
                </button>
              ))}
        ...
        >
          <div className="homepage-editor__inspector-field">
            ...
            <Input
              ...
              maxLength={FULL_BLEED_CONTRACT.content.limits.altText}
              placeholder="描述海报中的珠宝或场景"
            />
          </div>
        </InspectorSection>
      </div>
...
function DoublePosterInspector({
  ...
  const detailFocusX = Math.min(
    100,
    Math.max(0, Number(props.detailFocusX ?? 50)),
  );
  const detailFocusY = Math.min(
    100,
    Math.max(0, Number(props.detailFocusY ?? 50)),
  );
  ...
  return (
    ...
    >
      <InspectorHeader
        title={getModuleDisplayName("双图海报", props)}
        device={device}
        onClose={close}
      />
      <div className="homepage-editor__inspector-scroll">
        <p className="homepage-editor__properties-helper">
          {DOUBLE_POSTER_CONTRACT.purpose}
        </p>
        <ContractStatusBanner status={status} />
        ...
        <InspectorSection title="文字内容" resetKey={props.id}>
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.number || "").length}/
                {DOUBLE_POSTER_CONTRACT.content.limits.number}
              </span>
            </label>
            <Input
              value={props.number || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.label || "").length}/
                {DOUBLE_POSTER_CONTRACT.content.limits.label}
              </span>
            </label>
            <Input
              value={props.label || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.title || "").length}/
                {DOUBLE_POSTER_CONTRACT.content.limits.title}
              </span>
            </label>
            <Input
              value={props.title || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                ...
                {DOUBLE_POSTER_CONTRACT.content.limits.description}
              </span>
            </label>
            <Input.TextArea
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.actionText || "").length}/
                {DOUBLE_POSTER_CONTRACT.content.limits.actionText}
              </span>
            </label>
            <Input
              value={props.actionText || ""}
        ...
        <InspectorSection title="主海报 · 4:3" resetKey={props.id}>
          <MediaPickerField
            ...
            placeholder="上传主海报"
            previewAspectRatio={
              DOUBLE_POSTER_CONTRACT.canvas.mainMediaAspectRatio
            }
          ...
          {props.mainImage ? (
            <div className="homepage-editor__inspector-field">
              ...
              <FocusPicker
                ...
                safeArea
                onChange={(x, y) => update({ mainFocusX: x, mainFocusY: y })}
              />
            </div>
          ) : null}
        </InspectorSection>
        ...
        <InspectorSection title="细节海报 · 4:5" resetKey={props.id}>
          <MediaPickerField
            ...
            placeholder="上传细节海报"
            previewAspectRatio={
              DOUBLE_POSTER_CONTRACT.canvas.detailMediaAspectRatio
            }
          ...
          {props.detailImage ? (
            <div className="homepage-editor__inspector-field">
              ...
              <FocusPicker
                ...
                safeArea
                onChange={(x, y) =>
                  update({ detailFocusX: x, detailFocusY: y })
                }
              />
            </div>
          ) : null}
        ...
        >
          <div className="homepage-editor__inspector-option-group">
            <div>
              ...
              <span>移动端始终先展示主图，再展示细节图与文字。</span>
            </div>
            <div
              className="homepage-editor__inspector-segmented"
              role="group"
            ...
            >
              {[
                ...
                <button
                  ...
                  type="button"
                  className={
                    (props.layout || "mainLeft") === value ? "is-active" : ""
                  }
                  aria-pressed={(props.layout || "mainLeft") === value}
          ...
          <div className="homepage-editor__layout-rule">
            <strong>画布标准</strong>
            <span>主图固定4:3，细节图固定4:5</span>
            <span>桌面8/4栏；移动端上下排列，不使用视口高度撑大模块</span>
          </div>
        </InspectorSection>
        ...
        >
          <div className="homepage-editor__inspector-field">
            ...
            <Input
              value={props.mainAltText || ""}
              onChange={(event) => update({ mainAltText: event.target.value })}
          ...
          <div className="homepage-editor__inspector-field">
            <label>细节图替代文字</label>
            <Input
              value={props.detailAltText || ""}
              onChange={(event) =>
                update({ detailAltText: event.target.value })
              }
              maxLength={DOUBLE_POSTER_CONTRACT.content.limits.altText}
              placeholder="描述细节图内容"
            />
...
function ProductRowFlatSection({
  ...
  return (
    <section
      className="homepage-editor__product-row-section"
      aria-label={title}
    >
      ...
      <div className="homepage-editor__product-row-section-body">
        {children}
      </div>
    </section>
  );
...
function ProductRowInspector({
  ...
  const updateProducts = (nextIds: number[]) => {
    if (nextIds.length > PRODUCT_ROW_CONTRACT.content.maxProducts) {
      message.warning(
        `产品展示行最多选择 ${PRODUCT_ROW_CONTRACT.content.maxProducts} 件商品`,
      );
      return;
    }
  ...
  return (
    <section
      className="homepage-editor__inspector homepage-editor__product-row-inspector"
      data-active-device={device}
      aria-label="作品陈列模块属性"
    >
      <InspectorHeader
        title={getModuleDisplayName("产品展示行", props)}
        device={device}
        onClose={close}
      />
      <div className="homepage-editor__inspector-scroll">
        <p className="homepage-editor__properties-helper">
          {PRODUCT_ROW_CONTRACT.purpose}
        </p>
        <ContractStatusBanner status={status} />
        ...
        <ProductRowFlatSection title="模块基础内容">
          <div className="homepage-editor__inspector-field">
            ...
            <Input
              ...
              onChange={(event) => update({ moduleName: event.target.value })}
              maxLength={24}
              placeholder="默认使用作品陈列"
            />
          </div>
          <div className="homepage-editor__inspector-field">
            <label>模块样式</label>
            <div
              className="homepage-editor__product-row-mode-picker"
              role="group"
              aria-label="商品展示模式"
            >
              <button
              ...
              >
                <span className="homepage-editor__product-row-mode-preview">
                  <img
                    src="/svg/template-lookbook.svg"
                    alt="画册展示版式预览"
                  />
                </span>
                <span>
                  <strong>画册展示</strong>
                  <small>突出图片与作品氛围</small>
                </span>
              </button>
              <button
                type="button"
              ...
              >
                <span className="homepage-editor__product-row-mode-preview">
                  <img
                    src="/svg/template-product-row.svg"
                    alt="标准选款版式预览"
                  />
                </span>
                <span>
                  <strong>标准选款</strong>
                  <small>显示价格与详情入口</small>
                </span>
              </button>
            </div>
            <span className="homepage-editor__product-row-mode-note">
              选择样式会应用一组推荐配置，下方仍可逐项调整。
            </span>
          </div>
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.title || "").length}/
                {PRODUCT_ROW_CONTRACT.content.limits.title}
              </span>
            </label>
            <Input
              value={props.title || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.subtitle || "").length}/
                {PRODUCT_ROW_CONTRACT.content.limits.subtitle}
              </span>
            </label>
            <Input
              value={props.subtitle || ""}
        ...
        >
          <ProductIdsField
            ...
            maxProducts={PRODUCT_ROW_CONTRACT.content.maxProducts}
          />
        </ProductRowFlatSection>

        <ProductRowFlatSection
          title="陈列布局"
          description="桌面端与手机端设置同时显示，不随当前预览设备隐藏。"
        >
          <div className="homepage-editor__product-row-setting">
            <div>
              <strong>桌面端列数</strong>
              <span>控制电脑端每行展示的商品数量。</span>
            </div>
            <div
              className="homepage-editor__inspector-segmented is-three"
              role="group"
            ...
            >
              {[
                ...
                <button
                  ...
                  type="button"
                  className={
                    (props.layout || "grid-3") === value ? "is-active" : ""
                  }
                  aria-pressed={(props.layout || "grid-3") === value}
                  onClick={() => update({ layout: value })}
                >
                  {label}
                </button>
            ...
            >
              {[1, 2].map((value) => (
                <button
                  ...
                  type="button"
                  className={
                    (props.mobileColumns === 1 ? 1 : 2) === value
                      ? "is-active"
                      : ""
                  }
                  aria-pressed={(props.mobileColumns === 1 ? 1 : 2) === value}
                  onClick={() => update({ mobileColumns: value })}
                >
                  {value}列
                </button>
              ))}
          ...
          <div className="homepage-editor__product-row-setting">
            <div>
              <strong>商品图片比例</strong>
              <span>3:4适合珠宝画册，1:1适合统一商品主图。</span>
            </div>
            <div
              className="homepage-editor__inspector-segmented"
              role="group"
            ...
            >
              {[
                ...
                <button
                  ...
                  type="button"
                  className={
                    (props.imageRatio || "3:4") === value ? "is-active" : ""
                  }
                  aria-pressed={(props.imageRatio || "3:4") === value}
                  onClick={() => update({ imageRatio: value })}
                >
                  {label}
                </button>
              ))}
          ...
          <div className="homepage-editor__layout-rule">
            ...
            <span>
              {device === "desktop"
                ? `电脑端 · ${(props.layout || "grid-3").replace("grid-", "")}列`
                : `手机端 · ${props.mobileColumns === 1 ? 1 : 2}列`}
            </span>
            <span>图片比例：{props.imageRatio || "3:4"}</span>
          </div>
        </ProductRowFlatSection>
        ...
        <ProductRowFlatSection title="商品信息">
          <div className="homepage-editor__product-row-setting">
            ...
            >
              <button
                type="button"
                className={props.showPrice !== false ? "is-active" : ""}
                aria-pressed={props.showPrice !== false}
                onClick={() => update({ showPrice: true })}
              >
                显示价格
              </button>
              <button
                ...
                className={props.showPrice === false ? "is-active" : ""}
                aria-pressed={props.showPrice === false}
                onClick={() => update({ showPrice: false })}
              >
                隐藏价格
              </button>
            </div>
          ...
          <div className="homepage-editor__product-row-setting">
            <div>
              <strong>操作样式</strong>
              <span>三种样式均进入商品详情，不伪装成立即购买。</span>
            </div>
            <div
              className="homepage-editor__inspector-segmented is-three"
              role="group"
            ...
            >
              {[
                ...
                <button
                  key={value}
                  type="button"
                  className={actionStyle === value ? "is-active" : ""}
                  aria-pressed={actionStyle === value}
                  onClick={() =>
                    update({
                      actionStyle: value,
                      showButton: value === "button",
                      buttonText:
                        value === "button" ? "查看详情" : props.buttonText,
                    })
                  }
                >
                  {label}
        ...
        <ProductRowFlatSection title="模块背景">
          <div className="homepage-editor__product-row-setting">
            <div>
              <strong>背景预设</strong>
              <span>限定品牌中性色，避免不同模块出现杂乱配色。</span>
            </div>
            <div
              className="homepage-editor__inspector-segmented is-three"
              role="group"
            ...
            >
              {[
                ...
                <button
                  ...
                  type="button"
                  className={
                    (props.bgColor || "#FCFCFB").toUpperCase() === value
                      ? "is-active"
                      : ""
                  }
                  aria-pressed={
...
function FeaturedProductInspector({
  ...
  return (
    ...
    >
      <InspectorHeader
        title={getModuleDisplayName("单品焦点推荐", props)}
        device={device}
        onClose={close}
      />
      <div className="homepage-editor__inspector-scroll">
        <p className="homepage-editor__properties-helper">
          {FEATURED_PRODUCT_CONTRACT.purpose}
        </p>
        <ContractStatusBanner status={status} />
        ...
        <InspectorSection title="主推商品 · 1件" resetKey={props.id}>
          <ProductIdsField
            ...
            onChange={(ids) =>
              update({ productId: Number(ids[ids.length - 1]) || 0 })
            }
            maxProducts={1}
          />
          <p className="homepage-editor__section-note">
            商品主图、名称和价格读取真实商品数据；画布与发布页保持同步。
          </p>
        </InspectorSection>
        ...
        <InspectorSection title="内容表达" resetKey={props.id}>
          <div className="homepage-editor__inspector-field">
            <label>
              眉题
              <span className="homepage-editor__inspector-count">
                {(props.eyebrow || "").length}/
                {FEATURED_PRODUCT_CONTRACT.content.limits.eyebrow}
              </span>
            </label>
            <Input
              value={props.eyebrow || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.title || "").length}/
                {FEATURED_PRODUCT_CONTRACT.content.limits.title}
              </span>
            </label>
            <Input
              value={props.title || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                ...
                {FEATURED_PRODUCT_CONTRACT.content.limits.summary}
              </span>
            </label>
            <Input.TextArea
              value={props.summary || ""}
        ...
        >
          ...
          {props.secondaryText ? (
            <div className="homepage-editor__inspector-field">
              <label htmlFor={`featured-secondary-${props.id}`}>
                站内页面 <em>必填</em>
              </label>
              <select
                id={`featured-secondary-${props.id}`}
                value={props.secondaryLink || ""}
                onChange={(event) =>
                  update({ secondaryLink: event.target.value })
                }
              >
                ...
                {editorPages.map((page) => (
                  <option key={page.key} value={page.publicPath}>
                    {page.label}
                  </option>
                ))}
              </select>
        ...
        >
          <div className="homepage-editor__inspector-option-group">
            <div>
              <strong>桌面端图文顺序</strong>
              <span>移动端始终先展示商品图，再展示文字和行动入口。</span>
            </div>
            <div
              className="homepage-editor__inspector-segmented"
              role="group"
            ...
            >
              {[
                ...
                <button
                  ...
                  type="button"
                  className={
                    (props.layout || "imageLeft") === value ? "is-active" : ""
                  }
                  aria-pressed={(props.layout || "imageLeft") === value}
                  onClick={() => update({ layout: value })}
                >
                  {label}
                </button>
          ...
          <div className="homepage-editor__layout-rule">
            <strong>画布标准</strong>
            <span>商品主图固定3:4，桌面双栏，移动端上下排列</span>
            <span>商品详情始终是主行动，预约只作为次要入口</span>
          </div>
        </InspectorSection>
      </div>
      <InspectorDraftActions
...
function CategoryCardsInspector({
  ...
  const updateCards = (nextCards: Array<Record<string, any>>) =>
    update({ categories: nextCards });
  const updateCard = (index: number, patch: Record<string, any>) =>
    updateCards(
      cards.map((card: Record<string, any>, cardIndex: number) =>
  ...
  return (
    ...
    >
      <InspectorHeader
        title={getModuleDisplayName("分类卡片", props)}
        device={device}
        onClose={close}
      />
      <div className="homepage-editor__inspector-scroll">
        <p className="homepage-editor__properties-helper">
          {CATEGORY_CARDS_CONTRACT.purpose}
        </p>
        <ContractStatusBanner status={status} />
        ...
        <InspectorSection title="模块概况" resetKey={props.id}>
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.title || "").length}/
                {CATEGORY_CARDS_CONTRACT.content.limits.title}
              </span>
            </label>
            <Input
              value={props.title || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.subtitle || "").length}/
                {CATEGORY_CARDS_CONTRACT.content.limits.subtitle}
              </span>
            </label>
            <Input.TextArea
              value={props.subtitle || ""}
        ...
        >
          ...
          <div className="homepage-editor__layout-rule">
            <strong>画布标准</strong>
            <span>全部卡片固定3:4，桌面2/3/4列，移动端单列</span>
            <span>每张卡片只承担一次分类导航</span>
          </div>
        </InspectorSection>
      </div>
      <InspectorDraftActions
...
function AppointmentInspector({
  ...
  return (
    ...
    >
      <InspectorHeader
        title={getModuleDisplayName("预约入口", props)}
        device={device}
        onClose={close}
      />
      <div className="homepage-editor__inspector-scroll">
        <p className="homepage-editor__properties-helper">
          {APPOINTMENT_CONTRACT.purpose}
        </p>
        <ContractStatusBanner status={status} />
        ...
        <InspectorSection title="行动内容" resetKey={props.id}>
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.title || "").length}/
                {APPOINTMENT_CONTRACT.content.limits.title}
              </span>
            </label>
            <Input
              value={props.title || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label>
              ...
              <span className="homepage-editor__inspector-count">
                {(props.subtitle || "").length}/
                {APPOINTMENT_CONTRACT.content.limits.subtitle}
              </span>
            </label>
            <Input.TextArea
              value={props.subtitle || ""}
          ...
          <div className="homepage-editor__inspector-field">
            <label htmlFor={`appointment-page-${props.id}`}>
              预约页面 <em>必填</em>
            </label>
            <select
              id={`appointment-page-${props.id}`}
              value={props.linkUrl || ""}
              onChange={(event) => update({ linkUrl: event.target.value })}
            >
              <option value="" disabled>
        ...
        <InspectorSection title="背景视觉" resetKey={props.id}>
          <MediaPickerField
            ...
            spec={IMAGE_SPECS.fullBleed.desktop}
            placeholder="可选；留空使用纯色背景"
            previewAspectRatio={
              APPOINTMENT_CONTRACT.canvas.backgroundAspectRatio
            }
            previewFocus={{ x: focusX, y: focusY }}
          />
          {props.backgroundImage ? (
            <FocusPicker
              ...
              focusX={focusX}
              focusY={focusY}
              aspectRatio={APPOINTMENT_CONTRACT.canvas.backgroundAspectRatio}
              safeArea
              onChange={(x, y) => update({ focusX: x, focusY: y })}
            />
          ) : null}
        ...
        >
          ...
          >
            {[
              ...
              <button
                key={value}
                type="button"
                className={(props.tone || "dark") === value ? "is-active" : ""}
                aria-pressed={(props.tone || "dark") === value}
          ...
          <div className="homepage-editor__layout-rule">
            <strong>行动规则</strong>
            <span>预约按钮始终是唯一主行动</span>
            <span>电话为可选次要入口，不增加第三个按钮</span>
          </div>
        </InspectorSection>
...
function HotspotInspector({
  ...
  const imageSpec =
    device === "mobile"
      ? IMAGE_SPECS.hotspot.mobile
      : IMAGE_SPECS.hotspot.desktop;
  const aspectRatio =
    device === "mobile"
      ? HOTSPOT_CONTRACT.canvas.mobileMediaAspectRatio
      : HOTSPOT_CONTRACT.canvas.desktopMediaAspectRatio;
  const currentHotspots = Array.isArray(props[hotspotField])
    ? props[hotspotField]
    : [];
  const desktopHotspots = Array.isArray(props.hotspots) ? props.hotspots : [];

  const updateHotspots = (next: Array<Record<string, any>>) =>
    update({ [hotspotField]: next });
  const updateHotspot = (itemIndex: number, patch: Record<string, any>) => {
    updateHotspots(
      currentHotspots.map((item: Record<string, any>, index: number) =>
  ...
  const addHotspot = () => {
    if (currentHotspots.length >= HOTSPOT_CONTRACT.content.maxHotspots) {
      message.warning(
        `每个设备最多配置 ${HOTSPOT_CONTRACT.content.maxHotspots} 个热区`,
      );
      return;
    }
    ...
    updateHotspots([
      ...currentHotspots,
      { label: "", x: offset, y: offset, width: 24, height: 18, link: "" },
    ]);
  };
  ...
  return (
    ...
    >
      <InspectorHeader
        title={getModuleDisplayName("热区图", props)}
        device={device}
        onClose={close}
      />
      <div className="homepage-editor__inspector-scroll">
        ...
        >
          {device === "mobile" && !props.mobileImage ? (
            <div className="homepage-editor__media-status is-fallback">
              <ExclamationCircleOutlined />
              <div>
                <strong>当前复用桌面端底图</strong>
                <span>上传3:4移动图后，请重新校准移动端热区。</span>
              </div>
            </div>
          ) : null}
          <MediaPickerField
            ...
            spec={imageSpec}
            placeholder={
              device === "mobile" ? "上传移动端热区底图" : "上传桌面端热区底图"
            }
            previewAspectRatio={aspectRatio}
          />
        </InspectorSection>
        ...
        >
          ...
          desktopHotspots.length > 0 ? (
            <button
              type="button"
              className="homepage-editor__copy-device-config"
              onClick={() =>
                update({ mobileHotspots: cloneModuleProps(desktopHotspots) })
              }
            >
              复制桌面端热区后校准
            </button>
          ...
          <div className="homepage-editor__hotspot-list">
            {currentHotspots.map(
              (item: Record<string, any>, itemIndex: number) => (
                <article
                  key={itemIndex}
                  className="homepage-editor__hotspot-card"
                >
                  <header>
                    <strong>热区 {itemIndex + 1}</strong>
                    <div>
                      <button
                        type="button"
                        disabled={itemIndex === 0}
                        onClick={() => moveHotspot(itemIndex, -1)}
                      >
                        上移
                      </button>
                      <button
                        type="button"
                        disabled={itemIndex === currentHotspots.length - 1}
                        onClick={() => moveHotspot(itemIndex, 1)}
                      >
                        下移
                      </button>
                  ...
                  <div className="homepage-editor__inspector-field">
                    <label>
                      ...
                      <span className="homepage-editor__inspector-count">
                        {(item.label || "").length}/
                        {HOTSPOT_CONTRACT.content.limits.label}
                      </span>
                    </label>
                    <Input
                      size="small"
                      value={item.label || ""}
                      onChange={(event) =>
                        updateHotspot(itemIndex, { label: event.target.value })
                      }
                      maxLength={HOTSPOT_CONTRACT.content.limits.label}
                      placeholder="例如 查看系列"
                    />
                  </div>
                  <div className="homepage-editor__inspector-field">
                    ...
                    <Input
                      ...
                      value={item.link || ""}
                      onChange={(event) =>
                        updateHotspot(itemIndex, { link: event.target.value })
                      }
                      placeholder="例如 /products/123"
                      status={item.link?.trim() ? undefined : "error"}
                    />
                  </div>
          ...
          <button
            type="button"
            className="homepage-editor__add-hotspot"
            disabled={
              currentHotspots.length >= HOTSPOT_CONTRACT.content.maxHotspots
            }
            onClick={addHotspot}
          >
            ＋ 添加热区
        ...
        >
          <div className="homepage-editor__layout-rule">
            ...
            <span>移动：底图3:4，独立移动热区坐标</span>
            <span>未配置移动热区时暂时复用桌面坐标，并给出发布提醒</span>
          </div>
        </InspectorSection>
      </div>
      <InspectorDraftActions
