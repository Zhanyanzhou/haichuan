...
import type { MaterialType, ProductDetailBlock, ProductImage, ProductSKU, ProductStatus, ShippingTemplate } from "@/types";
...
const unwrap = <T,>(value: any): T => value?.data?.data ?? value?.data ?? value;
const materials = [
...
export default function ProfessionalProductEditor() {
  ...
  useEffect(() => {
    ...
    Promise.all([categoryApi.getManageTree(), shippingTemplateApi.list(), productRequest])
      .then(([categoryRes, templateRes, productRes]) => {
        if (!mounted) return;
        const loadedTemplates = unwrap<ShippingTemplate[]>(templateRes);
        setCategories(flattenCategories(unwrap<any[]>(categoryRes)));
        ...
        if (!editingId) {
          ...
          const product = unwrap<any>(productRes);
  ...
  const uploadPending = async (productId: number) => {
    ...
    for (let index = 0; index < mediaToUpload.length; index += 1) {
      ...
      const uploaded = unwrap<any[]>(await uploadApi.uploadProductImage(media.file))[0];
      if (!uploaded?.storageKey) throw new EditorUserError("图片上传失败，请重新选择图片后重试");
      const created = unwrap<any>(await productApi.addImage(productId, {
  ...
  const refreshProduct = async (productId: number) => {
    const refreshed = unwrap<any>(await productApi.getById(productId));
  ...
  const save = async (intent: "draft" | "primary" = "draft") => {
    ...
    try {
      ...
      const requiresPublishReadiness = currentStatus === "PUBLISHED"
        || (intent === "primary" && ["IMMEDIATE", "SCHEDULED"].includes(values.publishMode));
      if (requiresPublishReadiness && images.length + pendingMedia.length === 0) {
        throw new EditorUserError("上架商品必须至少有一张商品图片");
      }
      if (intent === "primary" && values.publishMode === "SCHEDULED" && !values.scheduledPublishAt) {
        throw new EditorUserError("请选择定时上架时间");
      }
      if (requiresPublishReadiness) {
        const hasPricedSku = currentProductId
          ? skus.some((sku) => sku.isActive && Number(sku.price) > 0)
          : Number(values.price) > 0;
        if (!hasPricedSku) throw new EditorUserError("上架前请至少配置一个已启用且价格大于 0 的 SKU");
      }
      const contentPayload = buildContentPayload(values);
      if (!productId) {
        const created = unwrap<any>(await productApi.create({
          ...contentPayload,
          status: "DRAFT",
          publishMode: "WAREHOUSE",
      ...
      if (Array.isArray(error?.errorFields)) {
        ...
        messageApi.warning(requiresFullValidation ? "请先完善全部必填信息" : "保存草稿至少需要选择类目并填写货号");
        if (firstField) {
      ...
      messageApi.error(getSafeAdminErrorMessage(error, "商品保存失败，请检查填写内容后重试。"));
      return false;
    } finally {
      setSaving(false);
  ...
  const refreshSkus = async () => {
    ...
    const result = unwrap<any>(await productApi.getById(currentProductId));
    setSkus(result.skus || []);
  ...
  const saveSku = async () => {
    ...
    try {
      ...
      if (!Array.isArray(error?.errorFields)) {
        messageApi.error(getSafeAdminErrorMessage(error, "SKU 保存失败，请检查规格信息后重试。"));
      }
    } finally {
      setSkuSaving(false);
    }
  ...
  const saveTemplate = async () => {
    ...
    try {
      ...
      const created = unwrap<ShippingTemplate>(await shippingTemplateApi.create(values));
  ...
  return (
    <div className="pro-editor">
      ...
      <header className="pro-editor__tabs">
        ...
        <label className="pro-editor__required"><Switch checked={requiredOnly} onChange={setRequiredOnly} /> 只看必填</label>
      </header>

      <Form name="product-editor-main" form={form} initialValues={defaultValues} layout="vertical" disabled={saving} onValuesChange={markDirty} className="pro-editor__form">
        ...
        <section id="media" className="pro-editor__card">
          ...
          <div className="pro-editor__detail-head"><div><strong>商品详情 <i>*</i></strong><span>使用上下按钮调整阅读顺序，桌面端与移动端共用同一内容源</span></div><Space><Button onClick={addImageBlock}>添加图片</Button><Button onClick={addTextBlock}>添加文字</Button></Space></div>
        ...
        <section id="basic" className="pro-editor__card">
          ...
          <Form.Item name="condition" label="商品类型" className="pro-editor__horizontal-field" rules={[{ required: true }]}><Radio.Group><Radio value="NEW">全新</Radio><Radio value="SECOND_HAND">二手</Radio></Radio.Group></Form.Item>
          ...
          <div className="pro-editor__attributes">
            ...
            <Form.Item name="materialType" label="主要材质" rules={[{ required: true }]}><Select options={materials} /></Form.Item>
            {!requiredOnly && <>
            ...
            <Form.Item name="purchaseRegion" label="采购地区" rules={[{ required: true }]} className="pro-editor__span-3"><Radio.Group><Radio value="MAINLAND">中国内地（大陆）</Radio><Radio value="CROSS_BORDER">中国港澳台地区及其他国家和地区</Radio></Radio.Group></Form.Item>
          </div>
        </section>
        ...
        <section id="sales" className="pro-editor__card">
          ...
          <div className="pro-editor__sales-grid">
            <Form.Item name="salesMode" label="销售方式" rules={[{ required: true }]} extra={!commerceEnabled ? "交易功能未启用，因此暂不可选择“直接购买”。" : undefined}><Radio.Group><Radio value="DISPLAY_ONLY">仅展示</Radio><Radio value="SELECTION">选款咨询</Radio><Radio value="APPOINTMENT">预约到店</Radio><Tooltip title={commerceEnabled ? undefined : "交易功能当前未启用"}><Radio value="DIRECT_PURCHASE" disabled={!commerceEnabled}>直接购买</Radio></Tooltip><Radio value="CUSTOM_INQUIRY">定制咨询</Radio></Radio.Group></Form.Item>
            <Form.Item name="price" label="一口价" className="pro-editor__money-field" rules={[{ required: true, message: "请输入价格" }]}><InputNumber min={0} precision={2} suffix="元" /></Form.Item>
            ...
            <Form.Item label="库存扣减方式"><Radio checked>下单预占库存，确认收款后扣减</Radio><span className="pro-editor__hint">库存规则由订单与库存模块统一管理，不允许商品级切换。</span></Form.Item>
            <Form.Item noStyle shouldUpdate={(prev, next) => prev.salesMode !== next.salesMode}>{({ getFieldValue }) => getFieldValue("salesMode") === "DIRECT_PURCHASE" ? <Alert className="pro-editor__inventory-alert" type={commerceEnabled ? "warning" : "error"} showIcon message={commerceEnabled ? "直接购买商品保存后，请到库存管理确认可售 SKU 和库存数量。" : "交易功能当前未启用，请改用仅展示、选款咨询或预约到店。"} action={commerceEnabled ? <Button size="small" onClick={() => navigate("/admin/inventory")}>前往库存管理</Button> : undefined} /> : null}</Form.Item>
            <Form.Item name="publishMode" label="上架时间" rules={[{ required: true }]}><Radio.Group><Radio value="IMMEDIATE">立刻上架</Radio><Radio value="SCHEDULED">定时上架</Radio><Radio value="WAREHOUSE">放入仓库</Radio></Radio.Group></Form.Item>
            ...
            <Form.Item name="visibility" label="可见范围" rules={[{ required: true, message: "请选择可见范围" }]}><Select options={[{ value: "PUBLIC", label: "公开宣传款（游客可见）" }, { value: "MEMBER", label: "登录会员可见" }, { value: "PARTNER", label: "合作商家专属" }, { value: "INTERNAL", label: "仅内部可见" }]} /></Form.Item>
          ...
          <div className="pro-editor__sku-panel">
            <div className="pro-editor__title-row">
              <div><h3>SKU 规格</h3><span>{currentProductId ? `${skus.filter((sku) => sku.isActive).length} 个在用；库存数量请在库存管理维护` : "先保存草稿，再配置规格与价格"}</span></div>
              <Button icon={<PlusOutlined />} disabled={!currentProductId} onClick={() => openSkuModal()}>添加 SKU</Button>
            </div>
            <Table<ProductSKU>
              rowKey="id"
              size="small"
              ...
              columns={[
                ...
                { title: "价格", dataIndex: "price", render: (value: number) => `¥ ${Number(value || 0).toFixed(2)}` },
                { title: "状态", dataIndex: "isActive", render: (value: boolean) => <Tag color={value ? "green" : "default"}>{value ? "启用" : "停用"}</Tag> },
                { title: "操作", render: (_value: unknown, sku: ProductSKU) => <Space size={4}><Button type="link" size="small" onClick={() => openSkuModal(sku)}>编辑</Button><Button type="link" size="small" onClick={() => void toggleSkuActive(sku)}>{sku.isActive ? "停用" : "启用"}</Button></Space> },
              ]}
              scroll={{ x: 760 }}
            />
          </div>
        ...
        <section id="delivery" className="pro-editor__card">
          ...
          <div className="pro-editor__delivery-grid">
            <Form.Item name="fulfillmentType" label="备货类型" rules={[{ required: true }]}><Radio.Group><Radio value="IN_STOCK">现货</Radio><Radio value="PREORDER">预售</Radio><Radio value="CUSTOM">定制</Radio></Radio.Group></Form.Item>
            <Form.Item name="dispatchTime" label="发货时间" rules={[{ required: true }]}><Radio.Group><Radio value="SAME_DAY">今日发</Radio><Radio value="WITHIN_24_HOURS">24小时内</Radio><Radio value="WITHIN_48_HOURS">48小时内</Radio><Radio value="OVER_48_HOURS">大于48小时</Radio><Radio value="CUSTOM">按约定</Radio></Radio.Group></Form.Item>
            ...
            <Form.Item name="deliveryMethods" label="提取方式" rules={[{ required: true, message: "请至少选择一种提取方式" }]}><Checkbox.Group options={[{ value: "EXPRESS", label: "物流配送" }, { value: "STORE_PICKUP", label: "到店自提" }, { value: "DEDICATED", label: "专人配送" }]} /></Form.Item>
            <Form.Item noStyle shouldUpdate={(prev, next) => prev.deliveryMethods !== next.deliveryMethods || prev.shippingTemplateId !== next.shippingTemplateId}>{({ getFieldValue }) => {
              ...
              return <>
                <Form.Item label="运费模板" required><div className="pro-editor__template-line"><Form.Item name="shippingTemplateId" noStyle rules={[{ required: true, message: "请选择运费模板" }]}><Select placeholder="请选择模板" options={templates.map((item) => ({ value: item.id, label: `${item.name}${item.isDefault ? "（默认）" : ""}` }))} /></Form.Item><Button icon={<PlusOutlined />} onClick={() => setTemplateOpen(true)}>新建</Button></div></Form.Item>
      ...
      <Modal rootClassName="pro-editor-modal" title="商品预览" width={760} open={previewOpen} onCancel={() => setPreviewOpen(false)} footer={null}>
        <div className="pro-editor__preview-dialog">
          <div className="pro-editor__preview-hero">
            ...
            <div><Tag>{selectedCategoryLabel || "珠宝"}</Tag><h3>{previewValues.name || "未命名商品"}</h3><strong>¥ {Number(previewValues.price || 0).toFixed(2)}</strong><p>{previewValues.brand || previewValues.materialType || "商品信息待完善"}</p></div>
          </div>
