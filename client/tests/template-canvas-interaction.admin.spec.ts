...
test("隔离画布新增：鼠标取景遵守屏幕阈值，松手保留预览，Esc恢复原取景", async ({ page }) => {
  ...
  const mouseHitPoint = async (target: Locator) => target.evaluate((element) => {
    const bounds = element.getBoundingClientRect();
    for (let row = 0; row < 10; row += 1) for (let column = 0; column < 10; column += 1) {
      const point = { x: bounds.left + bounds.width * (column + .5) / 10, y: bounds.top + bounds.height * (row + .5) / 10 };
      const hit = document.elementFromPoint(point.x, point.y);
      if (hit && (hit === element || element.contains(hit))) return point;
    }
    throw new Error("图片取景滑块没有可命中的屏幕坐标");
  });
  const point = await mouseHitPoint(slider);
  ...
  const recapturePoint = await mouseHitPoint(slider);
  await page.mouse.move(recapturePoint.x, recapturePoint.y); await page.mouse.down();
  ...
  await page.mouse.move(recapturePoint.x + 18, recapturePoint.y + 12, { steps: 5 });
  ...
  const activePoint = await mouseHitPoint(activeSlider);
  ...
  await page.mouse.move(activePoint.x, activePoint.y); await page.mouse.down();
