...
export class AttributesService {
  ...
  async create(body: any) {
    ...
    if (!/^[a-z][a-z0-9_-]*$/.test(key)) {
      throw new BadRequestException('属性键仅支持小写字母、数字、下划线与连字符');
    }
  ...
  async update(id: number, body: any) {
    ...
    return this.prisma.attribute.update({
      ...
      data: {
        ...
        ...(body?.isFilterable !== undefined ? { isFilterable: Boolean(body.isFilterable) } : {}),
  ...
  async remove(id: number) {
    await this.ensureAttribute(id);
    return this.prisma.attribute.update({ where: { id }, data: { isActive: false } });
  }
  ...
  async updateValue(valueId: number, body: any) {
    const val = await this.prisma.attributeValue.findUnique({ where: { id: valueId } });
  ...
  async removeValue(valueId: number) {
    const val = await this.prisma.attributeValue.findUnique({ where: { id: valueId } });
    if (!val) throw new NotFoundException('属性值不存在');
    return this.prisma.attributeValue.update({ where: { id: valueId }, data: { isActive: false } });
  }
