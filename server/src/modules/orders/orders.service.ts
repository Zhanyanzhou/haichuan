...
import { OrderStatus, OrderType, DeliveryStatus, CustomStage, Prisma } from '@prisma/client';
import { randomUUID } from 'crypto';
import { Cron, CronExpression } from '@nestjs/schedule';
import { TradeEventsService } from '../trade-events/trade-events.service';
import { OPERATOR_TYPE, TRADE_ENTITY_TYPE, TRADE_EVENT_TYPE, type OperatorContext } from '../trade-events/trade-events.constants';
...
export class OrdersService {
  ...
  private async expireReservationIfNeeded(
    ...
    await this.tradeEvents.record(tx, {
      orderId: order.id, entityType: TRADE_ENTITY_TYPE.INVENTORY, entityId: order.id,
      eventType: TRADE_EVENT_TYPE.STOCK_RELEASED, operator: { type: OPERATOR_TYPE.SYSTEM },
      reason: '超时未付款自动释放库存',
    });
    await this.tradeEvents.record(tx, {
      orderId: order.id, entityType: TRADE_ENTITY_TYPE.ORDER, entityId: order.id,
      eventType: TRADE_EVENT_TYPE.ORDER_CANCELLED, fromStatus: 'PENDING_PAYMENT', toStatus: 'CANCELLED',
      operator: { type: OPERATOR_TYPE.SYSTEM }, reason: '超时未付款自动取消',
    });
    return true;
  }
  ...
  private buildListWhere(params: OrderListParams): Prisma.OrderWhereInput {
    ...
    if (params.minAmount !== undefined || params.maxAmount !== undefined) {
      ...
      if (params.minAmount !== undefined && params.minAmount !== '') {
        where.finalAmount.gte = new Prisma.Decimal(params.minAmount as number | string);
      }
      if (params.maxAmount !== undefined && params.maxAmount !== '') {
        where.finalAmount.lte = new Prisma.Decimal(params.maxAmount as number | string);
      }
    }
    ...
    if (params.orderType && params.orderType !== 'all') where.orderType = params.orderType as OrderType;
    if (params.deliveryStatus && params.deliveryStatus !== 'all') where.deliveryStatus = params.deliveryStatus as DeliveryStatus;
    ...
    const [list, total] = await Promise.all([
      this.prisma.order.findMany({
        ...
        include: {
          ...
          payments: { select: { id: true, status: true, method: true, amount: true, createdAt: true }, orderBy: { createdAt: 'desc' }, take: 1 },
        },
  ...
  async findAllForExport(params: OrderListParams, limit = 1000) {
    ...
    return this.prisma.order.findMany({
      ...
      select: {
        orderNo: true, customerName: true, customerPhone: true,
        finalAmount: true, status: true, createdAt: true, paymentConfirmedAt: true,
      },
      orderBy: { createdAt: 'desc' },
    });
  }
  ...
  async findForCustomer(customerId: number) {
    ...
    return orders.map(({ payments, ...order }) => ({
      ...order,
      payments: payments.map(({ proofUrl, ...payment }) => ({ ...payment, hasProof: Boolean(proofUrl) })),
    }));
  }
  ...
  async create(data: {
    ...
    const operator: OperatorContext = data.operator ?? { type: OPERATOR_TYPE.SYSTEM };
    ...
    if (skus.length !== quantities.size) throw new BadRequestException('订单中包含不可直接购买的商品（仅"直接购买"商品可下单，其余请走咨询/预约）');
    ...
    const run = () => this.prisma.$transaction(async (tx) => {
      ...
      await this.tradeEvents.record(tx, {
        orderId: order.id, entityType: TRADE_ENTITY_TYPE.ORDER, entityId: order.id,
        eventType: TRADE_EVENT_TYPE.ORDER_CREATED, toStatus: 'PENDING_PAYMENT',
        operator, metadata: { totalAmount: totalAmount.toString(), itemCount: order.items.length },
      });
      await this.tradeEvents.record(tx, {
        orderId: order.id, entityType: TRADE_ENTITY_TYPE.INVENTORY, entityId: order.id,
        eventType: TRADE_EVENT_TYPE.STOCK_RESERVED, toStatus: 'PENDING_PAYMENT',
        operator, reason: `预占 ${order.items.length} 个商品行，24h 内未付款自动释放`,
      });
  ...
  async createFromQuotation(data: {
    ...
    const operator: OperatorContext = data.operator ?? { type: OPERATOR_TYPE.ADMIN };
    const customer = this.validateCustomerData(data);
    const reservedAt = new Date();
    const expiresAt = this.getReservationExpiry(reservedAt);
    const latestGoldPrice = await this.prisma.goldPrice.findFirst({ orderBy: { recordDate: 'desc' } });

    const context = { operator, customer, reservedAt, expiresAt, latestGoldPrice };
    // 独立事务 + orderNo 冲突重试（orderNo 事务内查 max+1，@unique 兜底）
    const run = () => this.prisma.$transaction((tx) => this.createOrderFromQuotationInTx(tx, data, context));
    for (let attempt = 0; attempt < 5; attempt++) {
      try {
        return await run();
      } catch (e) {
  ...
  async createOrderFromQuotationInTx(
    ...
    context: {
      operator: OperatorContext;
      customer: { customerName: string; customerPhone: string; address: string };
      reservedAt: Date;
      expiresAt: Date;
      latestGoldPrice: { price: Prisma.Decimal } | null;
    },
  ) {
    const { operator, customer, reservedAt, expiresAt, latestGoldPrice } = context;
    if (!data.items?.length) throw new BadRequestException('报价单无商品，不可转订单');

    // 金额统一用整数分计算后转 Decimal，规避浮点误差
    const toDecimal = (v: number | string) => new Prisma.Decimal(Math.round(Number(v) * 100)).div(100);
    const balanceCents = Math.max(0, Math.round(Number(data.finalAmount) * 100) - Math.round(Number(data.depositAmount || 0) * 100));

    const orderItems: Prisma.OrderItemUncheckedCreateWithoutOrderInput[] = data.items.map((it) => ({
      skuId: it.skuId,
    ...
    await this.tradeEvents.record(tx, {
      orderId: order.id, entityType: TRADE_ENTITY_TYPE.ORDER, entityId: order.id,
      eventType: TRADE_EVENT_TYPE.ORDER_CREATED, toStatus: 'PENDING_PAYMENT',
      operator, metadata: { totalAmount: order.totalAmount.toString(), itemCount: order.items.length, fromQuotationId: data.quotationId },
    });
    await this.tradeEvents.record(tx, {
      orderId: order.id, entityType: TRADE_ENTITY_TYPE.INVENTORY, entityId: order.id,
      eventType: TRADE_EVENT_TYPE.STOCK_RESERVED, toStatus: 'PENDING_PAYMENT',
      operator, reason: `报价单 #${data.quotationId} 转订单预占 ${order.items.length} 个商品行`,
    });
    return order;
  }

  async submitOfflinePaymentProof(customerId: number, orderId: number, proofKey: string, operator?: OperatorContext) {
    const actor: OperatorContext = operator ?? { type: OPERATOR_TYPE.CUSTOMER, id: customerId };
    const normalizedProofKey = proofKey?.trim();
    const paymentProofKeyPattern = new RegExp(`^${customerId}/\\d{4}/\\d{2}/\\d{2}/[0-9a-f-]{36}\\.(jpg|png|webp|gif)$`, 'i');
    if (!normalizedProofKey || !paymentProofKeyPattern.test(normalizedProofKey)) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (await this.expireReservationIfNeeded(tx, order.id)) {
        return { order: null, payment: null as null | { id: number; orderId: number; proofUrl: string | null } };
      }
      ...
      const payment = existingPayment
        ? await tx.payment.update({
          ...
          data: { method: 'bank_transfer', status: 'PENDING', proofUrl: normalizedProofKey, reviewedBy: null, reviewedAt: null, reviewNote: null },
        })
      ...
      await tx.order.update({
        where: { id: orderId },
        data: { paymentMethod: 'bank_transfer', paymentProof: normalizedProofKey },
      });
      return { order, payment };
    });
    if (!result.order) throw new BadRequestException('订单的库存保留已到期，请重新下单');

    // 凭证提交事件（事务外记录，避免重复读；失败仅日志）
    await this.tradeEvents.record(this.prisma, {
      orderId: result.order.id, entityType: TRADE_ENTITY_TYPE.PAYMENT, entityId: result.payment!.id,
      eventType: TRADE_EVENT_TYPE.PAYMENT_PROOF_SUBMITTED, operator: actor,
      metadata: { hasProof: true },
    });
    return result.payment;
  }

  async approveOfflinePayment(paymentId: number, reviewerId: number, reviewNote?: string, operator?: OperatorContext) {
    const actor: OperatorContext = operator ?? { type: OPERATOR_TYPE.ADMIN, id: reviewerId };
    return this.prisma.$transaction(async (tx) => {
      ...
      const consumed = await this.consumeStockReservations(tx, payment.orderId, now);
      if (consumed.count === 0) {
      ...
      // 交易事件：付款审核通过 + 库存消费
      await this.tradeEvents.record(tx, {
        orderId: payment.orderId, entityType: TRADE_ENTITY_TYPE.PAYMENT, entityId: paymentId,
        eventType: TRADE_EVENT_TYPE.PAYMENT_APPROVED, fromStatus: 'PENDING', toStatus: 'PAID',
        operator: actor, reason: reviewNote?.trim() || null,
      });
      await this.tradeEvents.record(tx, {
        orderId: payment.orderId, entityType: TRADE_ENTITY_TYPE.INVENTORY, entityId: payment.orderId,
        eventType: TRADE_EVENT_TYPE.STOCK_CONSUMED, operator: actor, reason: '确认收款，预占转为实扣',
      });
  ...
  async recordManualReceipt(data: {
    ...
    const actor: OperatorContext = data.operator ?? { type: OPERATOR_TYPE.ADMIN };
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // 按 type 同步订单已收金额字段
      const orderUpdate: Prisma.OrderUpdateInput = { paidAmount: { increment: payment.amount } };
      if (data.type === 'DEPOSIT') orderUpdate.paidDeposit = { increment: payment.amount };
      if (data.type === 'BALANCE') orderUpdate.paidBalance = { increment: payment.amount };

      // FULL 或 BALANCE 且订单待付款：推进到待发货 + 消费库存预占
      let consumed = false;
      if (order.status === 'PENDING_PAYMENT' && (data.type === 'FULL' || data.type === 'BALANCE')) {
        const result = await this.consumeStockReservations(tx, data.orderId, now);
        if (result.count === 0) {
          // 预占已超时释放，重新预占后立即消费（与 approveOfflinePayment 一致）
          const expiresAt = this.getReservationExpiry(now);
          for (const item of order.items) {
            await this.reserveStock(tx, data.orderId, item.skuId, item.quantity, expiresAt);
          }
      ...
      await this.tradeEvents.record(tx, {
        orderId: data.orderId, entityType: TRADE_ENTITY_TYPE.PAYMENT, entityId: payment.id,
        eventType: TRADE_EVENT_TYPE.PAYMENT_APPROVED, fromStatus: 'PENDING', toStatus: 'PAID',
        operator: actor, reason: data.reviewNote?.trim() || null,
        metadata: { method: data.method, type: data.type, manual: true, gatewayTradeNo: data.gatewayTradeNo || null },
      });
      if (consumed) {
        await this.tradeEvents.record(tx, {
          orderId: data.orderId, entityType: TRADE_ENTITY_TYPE.INVENTORY, entityId: data.orderId,
          eventType: TRADE_EVENT_TYPE.STOCK_CONSUMED, operator: actor, reason: '后台登记收款，预占转实扣',
        });
      }
  ...
  async rejectOfflinePayment(paymentId: number, reviewerId: number, reviewNote?: string, operator?: OperatorContext) {
    const actor: OperatorContext = operator ?? { type: OPERATOR_TYPE.ADMIN, id: reviewerId };
    ...
    if (updated.count === 0) throw new BadRequestException('该付款记录已被处理，请刷新后重试');

    await this.tradeEvents.record(this.prisma, {
      orderId: payment.orderId, entityType: TRADE_ENTITY_TYPE.PAYMENT, entityId: paymentId,
      eventType: TRADE_EVENT_TYPE.PAYMENT_REJECTED, fromStatus: 'PENDING', toStatus: 'FAILED',
      operator: actor, reason: reviewNote?.trim() || null,
    });
  ...
  async ship(
    id: number,
    data: { logisticsCompany: string; logisticsNo: string; internalNote?: string },
    operator?: OperatorContext,
  ) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (order.status !== 'PENDING_SHIP') {
        throw new BadRequestException('只有待发货订单可以发货（请先完成付款审核）');
      }
      // 防重复发货：已有非异常履约单则拒绝
      const existing = await tx.fulfillment.findFirst({
        where: { orderId: id, status: { in: ['PENDING_PICK', 'PENDING_CHECK', 'PENDING_SHIP', 'SHIPPED', 'DELIVERED'] } },
        select: { id: true, fulfillmentNo: true },
      });
      if (existing) {
        throw new BadRequestException(`订单已存在履约单 ${existing.fulfillmentNo}，不可重复发货`);
      }
      ...
      return tx.order.findUnique({ where: { id }, include: { fulfillments: true } });
    });
  }

  async updateStatus(
    id: number,
    data: { status: string; logisticsCompany?: string; logisticsNo?: string; internalNote?: string; operator?: OperatorContext },
  ) {
    const operator: OperatorContext = (data as { operator?: OperatorContext }).operator ?? { type: OPERATOR_TYPE.ADMIN };
    ...
    if (newStatus === 'COMPLETED') {
      ...
      await this.tradeEvents.record(this.prisma, {
        orderId: id, entityType: TRADE_ENTITY_TYPE.ORDER, entityId: id,
        eventType: TRADE_EVENT_TYPE.ORDER_COMPLETED, fromStatus: currentStatus, toStatus: newStatus,
        operator,
      });
      return updated;
    }
    ...
    if (newStatus === 'CANCELLED') {
      ...
      return this.prisma.$transaction(async (tx) => {
        const releasedCount = await this.releaseStockReservations(tx, id, cancelledAt);
        ...
        if (releasedCount > 0) {
          await this.tradeEvents.record(tx, {
            orderId: id, entityType: TRADE_ENTITY_TYPE.INVENTORY, entityId: id,
            eventType: TRADE_EVENT_TYPE.STOCK_RELEASED, operator,
            reason: `取消订单释放 ${releasedCount} 个预占`,
          });
        }
        await this.tradeEvents.record(tx, {
          orderId: id, entityType: TRADE_ENTITY_TYPE.ORDER, entityId: id,
          eventType: TRADE_EVENT_TYPE.ORDER_CANCELLED, fromStatus: currentStatus, toStatus: newStatus,
          operator, reason: data.internalNote || null,
        });
        return updated;
      });
    }
  ...
  async updateAmount(
    ...
    const fields: Array<{ key: string; src: keyof typeof data; label: string }> = [
    ...
    for (const f of fields) {
      ...
      if (!Number.isFinite(cents)) throw new BadRequestException(`${f.label} 必须为有效数字`);
      const decimal = new Prisma.Decimal(cents).div(100);
      before[f.label] = String((order as unknown as Record<string, unknown>)[f.key] ?? '');
    ...
    await this.tradeEvents.record(this.prisma, {
      orderId: id, entityType: TRADE_ENTITY_TYPE.ORDER, entityId: id,
      eventType: TRADE_EVENT_TYPE.ORDER_AMOUNT_EDITED, operator: actor,
  ...
  async updateAddress(id: number, address: string, operator?: OperatorContext) {
    ...
    if (!trimmed || trimmed.length > 500) throw new BadRequestException('请提供有效的收货地址');
    const order = await this.prisma.order.findUnique({ where: { id } });
    if (!order) throw new NotFoundException('订单不存在');
    ...
    const updated = await this.prisma.order.update({ where: { id }, data: { address: trimmed } });
    await this.tradeEvents.record(this.prisma, {
      orderId: id, entityType: TRADE_ENTITY_TYPE.ORDER, entityId: id,
      eventType: TRADE_EVENT_TYPE.ORDER_ADDRESS_EDITED, operator: actor,
  ...
  /** 修改内部备注（后台备注，不展示给客户） */
  async updateNote(id: number, internalNote: string, operator?: OperatorContext) {
    const actor: OperatorContext = operator ?? { type: OPERATOR_TYPE.ADMIN };
    const order = await this.prisma.order.findUnique({ where: { id } });
    if (!order) throw new NotFoundException('订单不存在');
    ...
    await this.tradeEvents.record(this.prisma, {
      orderId: id, entityType: TRADE_ENTITY_TYPE.ORDER, entityId: id,
      eventType: TRADE_EVENT_TYPE.ORDER_NOTE_EDITED, operator: actor,
  ...
  async confirmReceive(id: number, operator?: OperatorContext) {
    ...
    await this.tradeEvents.record(this.prisma, {
      orderId: id, entityType: TRADE_ENTITY_TYPE.ORDER, entityId: id,
      eventType: TRADE_EVENT_TYPE.ORDER_RECEIVED, fromStatus: 'SHIPPED', toStatus: 'RECEIVED',
      operator: actor,
    });
    return updated;
  }
  ...
  /** 修改销售顾问 */
  async updateSalesConsultant(id: number, salesConsultantId: number | null, operator?: OperatorContext) {
    const actor: OperatorContext = operator ?? { type: OPERATOR_TYPE.ADMIN };
    const order = await this.prisma.order.findUnique({ where: { id } });
    if (!order) throw new NotFoundException('订单不存在');
    ...
    await this.tradeEvents.record(this.prisma, {
      orderId: id, entityType: TRADE_ENTITY_TYPE.ORDER, entityId: id,
      eventType: TRADE_EVENT_TYPE.ORDER_CONSULTANT_CHANGED, operator: actor,
   ...
   */
  async advanceCustomStage(id: number, stage: CustomStage, operator?: OperatorContext) {
    const actor: OperatorContext = operator ?? { type: OPERATOR_TYPE.ADMIN };
    const order = await this.prisma.order.findUnique({ where: { id } });
    if (!order) throw new NotFoundException('订单不存在');
    ...
    await this.tradeEvents.record(this.prisma, {
      ...
      fromStatus: before ?? null, toStatus: stage, operator: actor,
    });
    return updated;
  }
  ...
  async findAnomalies() {
    ...
    const where: Prisma.OrderWhereInput = {
      OR: [
        ...
        { orderType: 'CUSTOM', customStage: { not: 'COMPLETED' }, createdAt: { lt: d30 } }, // 定制超期
        { deliveryStatus: 'ABNORMAL' }, // 物流异常
        { refunds: { some: { status: { in: ['PENDING', 'APPROVED', 'PROCESSING'] } } } }, // 退款处理中
      ],
    };
    ...
    const [list, total] = await Promise.all([
      this.prisma.order.findMany({
        where,
        include: { items: { select: { id: true, productNameSnapshot: true, productCodeSnapshot: true } } },
        orderBy: { createdAt: 'desc' },
        take: 200,
      }),
      this.prisma.order.count({ where }),
    ...
    const enriched = list.map((o) => {
      ...
      if (o.status === 'PENDING_PAYMENT' && o.createdAt < h24) reasons.push('长时间未付款');
      if (o.status === 'PENDING_SHIP' && o.paymentConfirmedAt && o.paymentConfirmedAt < h48) reasons.push('超时未发货');
      if (o.orderType === 'CUSTOM' && o.customStage !== 'COMPLETED' && o.createdAt < d30) reasons.push('定制超期');
  ...
  async getTradeOverview() {
    ...
    const [todayAgg, todayCount, paidAgg, finalAgg, refundedAgg, activeOrderCount, sourceGroup, typeGroup] = await Promise.all([
      ...
      this.prisma.order.count({ where: { status: { not: 'CANCELLED' } } }),
      this.prisma.order.groupBy({ by: ['source'], where: { status: { not: 'CANCELLED' } }, _count: { _all: true } }),
      this.prisma.order.groupBy({ by: ['orderType'], where: { status: { not: 'CANCELLED' } }, _count: { _all: true } }),
    ]);
    ...
    return {
      ...
      amount: {
        ...
        avgOrderValue: activeOrderCount > 0 ? finalAmount / activeOrderCount : 0,
      },
      distribution: {
        source: sourceGroup.map((g) => ({ source: g.source || '未知', count: g._count._all })),
        orderType: typeGroup.map((g) => ({ orderType: g.orderType, count: g._count._all })),
      },
    };
  }
}
