...
export class SmsService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    try {
      ...
      this.logger.warn(
        `阿里云短信 SDK 未安装或初始化失败（npm install 后生效）：${error instanceof Error ? error.message : error}`,
      );
    }
  }
  ...
  async sendVerificationCode(
    ...
    if (!this.client) {
      this.logger.warn(`[短信未发送·SMS 未配置] -> ${phone}`);
    ...
    try {
      ...
      this.logger.error(
        `阿里云短信受理失败 -> ${phone}: ${response?.body?.code} ${response?.body?.message}`,
      );
      return { delivered: false, reason: response?.body?.code || 'provider_rejected' };
    } catch (error) {
      this.logger.error(
        `阿里云短信发送异常 -> ${phone}: ${error instanceof Error ? error.message : error}`,
      );
