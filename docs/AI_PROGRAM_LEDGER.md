...
> 最后整理：2026-09-11（修正文档复核状态与候选发布范围）。本文只维护仍然有效的跨阶段工作包、阻断和下一动作；不保存 Git 指纹、测试数量、migration 数量、运行容器或历史批次日志。
...
| `HC-DATABASE-INSTALL` 空库安装与迁移兼容 | `BLOCKED` | 当前候选在隔离空库完整迁移并执行真实数据库测试；既有库升级与回退另有对应证据 | [2026-09-11 数据库安装验证](CURRENT_STATE.md#数据库安装验证2026-09-11)记录迁移 CHECK 与外键动作冲突。先形成兼容修复方案并核对所需授权，再验证空库和既有库；不得用 migrate resolve、改历史 SQL 或跳过门禁伪造通过 |
