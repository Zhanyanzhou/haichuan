...
> [!CAUTION]
> **历史运营盘点，不是当前上线清单。** 当前状态只查 `docs/CURRENT_STATE.md`，身份/行动/字段边界查 `docs/PUBLIC_ACCESS_MATRIX.md`，阶段验收查当前 `docs/acceptance/feature-completion-matrix.md` 并以本轮新鲜验证为准。下文“安全基本面扎实”“代码全部就绪”“全部改动未验证”等互相矛盾的旧结论均不具现行效力；不得据此启动容器、改环境、执行 migration、接入凭据、部署或宣称上线就绪。
>
> 历史盘点日期：2026-08-15；后续校正记录也仅作为历史证据保留。
>
> **2026-08-26 当前校正**：仓库现有备份脚本但没有可执行恢复脚本；备份默认同宿主机、明文、7 天保留且媒体归档非原子。完整 seed 会创建默认仓库和 5 条已发布样例商品，不适合作为生产首管理员初始化。微信支付证书路径虽已进入变量合同，Compose 尚无证书目录只读挂载。Notification/Delivery/Outbox 已形成订单创建与每笔支付确认的第一批本地候选，但目标库、真实 MySQL 抢占和 SMTP 送达未验证，外部投递默认关闭。`/en` 只有不可索引且不回退中文的 EN-A 安全轨道，正式英文内容未接入，候选 PageDocument revision 仍缺 locale/contentHash。以上事项均以 `docs/CURRENT_STATE.md` 与 `docs/plans/2026-08-26-closure-approval-packet.md` 为准。
...
- **支付网关抽象层**：微信 Native/H5 客户支付、查单、关单与原路退款代码路径已接入；支付宝保留后台当面付适配器、客户旅程和原路退款仍待后续批次。在线到账只认验签回调或主动查单，线下能力隔离为异常补录。

### 🟡 半成品

| 项         | 缺口                                                             | 档位   |
| ---------- | ---------------------------------------------------------------- | ------ |
| 支付联调   | 微信 APIv3 客户支付已有代码级与隔离测试，**尚未完成真实商户、正式域名和真实资金验收** | 解冻前 |
| 退款资金流 | 微信审核后原路退款已有发起、查询、验签回调代码路径；支付宝退款、真实商户联调和渠道对账未完成。在线退款禁止人工标记成功 | P2 |
...
- **页面 CMS 化**：Puck 构建器 6 页可视化编辑、草稿/发布预检/版本回滚/SSE 实时推送；模板数量、状态与页面适用范围以机器合同为准。
...
- **历史部署快照**：当时记录 docker-compose 五服务健康检查与 32 个迁移；当前新增 migration 及目标数据库状态未确认，不能沿用“迁移完整”结论。
...
本段原列出的 2026-08-15 验证与五项“待 git rm”清单已由后续批次处理或重新审计，不再是当前待办，也不得据此执行 migration、容器操作或文件删除。当前验证与删除范围只认本轮工作树、`docs/CURRENT_STATE.md` 和新鲜验收结果。
...
**凭据类**（只能在获批目标环境逐项核验，不能概括为“填入即上线”）：SMTP → 邮件通知｜ALIYUN_SMS 四项 → 短信验证码｜KUAIDI100 两项 → 物流轨迹｜支付宝三项、微信支付六项、正式 `SITE_BASE_URL`、回调与商户产品权限 → 在线资金联调｜WECHAT 开放平台三项 → 扫码登录｜OSS → CDN（当前零接线属新增开发）
...
| 二 数据库迁移 | ⚠️ 2026-08-15 历史结果；当前目标数据库和新增 migration 状态未确认，不得作为执行或上线依据 |
| 三 编排+备份  | ✅ 五容器 healthy；修复版 backup.sh 实测三产物齐全（旧轮次 PROCESS 权限/tar 误判两个错误均不再现）；**恢复演练通过**（表数 46=46、抽样 6 表一致、微信新列在位、临时库自清理）                                                                                                                                                   |
| 四 后台回归   | ✅ admin 视角全过（2026-08-15 浏览器实测）：登录落地/dashboard 统计正常（历史 400 不复现）/客户管理"回车应用"交互/设置页真实备份状态（16:51 那轮+8 产物）/AI 分类实测 503+诚实消息（GB-1 关闭）/当时机器合同登记模板的三档预览/评价页/建单弹窗 UI 联动（完整链路受阻于无在售商品=数据态非代码）。模板数量不从该历史记录继承；多角色落地页矩阵无其他角色账号未测。 |
...
**CSP 拦截外部字体域 → 品牌字体失效 + 编辑器整页崩溃**：`client/nginx.conf` 原 CSP 未放行 `fonts.googleapis.com` 与 `rsms.me`（Puck 编辑器样式的 `@import "inter.css"`）。部署形态下品牌衬线字体全部回退系统字体，且 @import 被 CSP 拦时 Chrome 将整个样式表标记失败 → Vite preload 抛错 → `/admin/editor/home` 崩溃到兜底页（dev 模式无 CSP 故从未暴露）。修复：CSP 放行两域（样式+字体二进制），重建容器复验通过（当时机器合同登记的模板均正常、console 零错误）。**遗留 P1**：品牌字体本地化——两个字体域国内均常不可达，公网上线前应下载 woff2 本地自托管并收紧 CSP 回 'self'。
