...
> 交易现状：已开放（`CUSTOMER_COMMERCE_ENABLED` 默认=true：前端加购/结算渲染 / CustomerCommerceGuard 放行 / `/cart`、`/checkout` 渲染真实页面；线上支付待商户凭据）
...
> **覆盖边界说明**：下列"通过"指 admin 视角后台回归 + 公开页面 + 容器部署形态；**未覆盖**：多角色落地页矩阵（无角色账号）、客户中心写流程（注册/忘记密码/下单类，按"不向真实库写测试数据"原则留人工）、建单完整链路（无在售商品）、>1MB 真实上传实测（nginx 配置已改，语义正确）。这些留待对应条件具备时补验。
