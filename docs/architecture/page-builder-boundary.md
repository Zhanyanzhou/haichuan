...
> 最近更新：2026-08-26 | 本文描述当前 Puck PageDocument 边界；模板数量、结构、页面角色、比例和能力必须以机器合同及生成产物为准。
...
模板库艺术指导素材只属于 `TEMPLATE_PREVIEW_CONTENT` 预览层，不得进入组件 `defaultProps`、推荐 PageDocument、正式草稿或业务 API。当前浏览器预览只加载 WebP 派生图；同目录 PNG 母版仅供内部编辑和归档，不是已授权正式媒体。正式页面媒体仍必须由内容负责人提供来源与授权记录，并通过上述发布门禁逐 URL 验证。
