...
| **首页/一级页装修** | PageDocument API | `GET /api/page-modules/document/published?pageKey=...` | `page_documents` + `page_document_revisions` | 未发布、无效或读取失败时仅安全短页 | **代码闭环，正式内容待验收** |
...
### ⚠️ 部分闭环 (3项)
- 首页（PageDocument API；未发布、无效或读取失败时只显示安全短页）
- 工作台 (API 调用有 400 错误)
- 首页内容槽 (API 可用，数据为空)
