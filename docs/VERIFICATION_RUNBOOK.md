...
| /admin/editor/home       | 模板库全量展示(不做模式过滤,2026-08-15 用户决策)、25 组件 Schema 面板、两档预览、发布校验、导入导出                        | 模板规范化   |

## 五、前台功能回归

| 页面              | 验证点                                                                                                                                                                               | 对应批次         |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------- |
| `/`（无发布文档） | 只显示中性安全短页，不出现编辑器种子、硬编码品牌长页或静态商品推荐                                                                                                                   | R5 内容收敛      |
...
含 public-access（公开访问+交易鉴权）、responsive、privacy-trust 等既有用例。
