...
### 2026-08-14 后端业务与交易协同

`de6fc45` `feat(server): 固化后台业务与交易协同方案`：保存当前已被前端路由与管理端页面引用的后端实现，共 70 个源码文件（5,742 行新增、453 行删改）。范围包括全局 JWT/角色认证、审计与健康检查；受控目录与媒体、会员/合作商可见性、选款咨询复核、合作商申请与推荐；以及订单、付款、履约、退款、售后、报价和交易事件的服务端协同模块。此批次不包含任何真实业务数据、数据库执行、密码/凭据重置脚本、迁移辅助脚本、CSV、Docker 覆盖、本机配置或依赖清单。

提交前已逐项复核暂存区路径及 `git diff --cached --check`。在相同完整工作树上，`npm run typecheck`、`npm run lint`、`npm run test:selection-inquiry`（20 项）、`npm run test:contracts`（26 项）、`npm run test:trade`（状态机 15 项、并发/金额/幂等 18 项、跨层契约 26 项）及 `npm run build` 均通过；构建未连接数据库。前端生产构建约 61 秒，Vite 报告 `antd` 与页面装修器运行时存在较大分包，仅为性能优化提示，非构建失败；后续应在包体专项中继续拆分/审查。

运行与上线边界：此提交仅保存源代码，不改变 `CUSTOMER_COMMERCE_ENABLED` 的默认冻结策略，也不执行 Prisma、migration、seed 或真实支付/订单操作。目录访问、合作商政策与交易域仍须在隔离环境使用真实角色令牌完成运行期联调；任何迁移执行必须由数据库负责人另行审批。

### 2026-08-14 客户与后台业务工作台

`9a17ae3` `feat(client): 固化客户与后台业务工作台`：保存已由应用路由直接引用的当前前端方案，共 86 个路径（5,410 行新增、1,419 行删改）。包括公开目录、商品详情/搜索、客户中心与合作商申请；后台目录、选款咨询、合作商申请和交易工作台；路由级角色控制、客户登录守卫、全局失败反馈、受控图片/目录数据接入、事件流重连，以及页面装修器前端收尾和 24 个被模板预览引用的 SVG。`/cart` 和 `/checkout` 仍重定向到咨询入口，未开放公开交易。

提交前已确认暂存区仅含前端源码与所引用的静态 SVG，并通过 `git diff --cached --check`。验证证据复用自同一未变更源码工作树中紧邻的后端批次：`npm run typecheck`、`npm run lint`、选款咨询 20 项、页面装修器 26 项、交易契约 59 项和 `npm run build` 均通过；构建未访问数据库。仍需在隔离环境完成不同后台角色、客户令牌、合作商审批和交易失败态的真实接口联调。

### 2026-08-14 关键业务静态验收

`fa33434` `test(contracts): 固化关键业务静态验收`：新增并接入页面装修器、选款咨询与交易域的 7 个只读静态契约脚本，根目录 `package.json` 仅新增统一的类型检查、Lint、契约与交易验证命令，不改变任何依赖版本。提交前运行全部 7 个脚本：页面装修器区块契约（26 种）与保存/发布闭环、选款咨询安全复核（20 项）与场景复核（15 项）、交易状态机（15 项）、并发/金额/幂等（18 项）、跨层契约（26 项）均通过。未执行数据库、迁移、seed、网络请求或真实数据写入。

审查中发现选款咨询场景测试的两处旧文本断言与当前实现不一致：当前服务端以 `validItems.length !== items.length` 拒绝不完整条目，并使用不带 `/api` 前缀的 `/products/catalog/...` 受控媒体路径，以避免前端 `SecureImage` 再拼接 API 前缀后产生 401。断言已按该已验证安全行为最小更新；未修改业务实现。Playwright 套件仍有一处固定 300ms 等待，未作为本次全量 E2E 通过证据，后续应改成可观察的网络/界面断言以减少偶发失败。

### 2026-08-14 发布边界与验收依据

`58fdac1` `docs: 固化发布边界与验收依据`：版本化 14 份当前架构、模块、数据/API、开发流程、公开访问矩阵、页面装修器能力矩阵和验收记录。公开访问矩阵明确保持“公开展示 + 选款/咨询转化”，交易、支付和资金相关公开入口继续冻结；Puck 审计文档明确历史已发布内容仍需人工逐页确认。提交前对候选文档进行了仅文件名的敏感关键字扫描并通过 `git diff --cached --check`；仅清理了 4 份新文档的行尾空白/末尾空行，未改动其结论。
...
### 2026-08-14 公开页面与装修器验收

`127ad9f` `test(e2e): 固化公开页面与装修器验收`：新增 Playwright 本机配置、公开访问/隐私/响应式/后台导航测试，以及页面装修器静态渲染契约脚本。配置默认只启动 `127.0.0.1` 的本地 Vite；真实 API、后台 storage state 和客户令牌均需显式环境变量才会启用。客户端静态渲染契约共通过 77 项（28 + 12 + 8 + 29）；本地 mock 公开 E2E 为 48 通过、14 条按环境条件跳过；首页网络夹具在非 mock 模式单独 2/2 通过。
...
### 2026-08-14 Puck 平板区块响应式补齐

`5c5badb` `fix(puck): 补齐平板区块响应式布局`：承接平板断点契约，仅提交 `CategoryCardsBlock`、`ImageTextBlock` 与 `SinglePosterSection` 的 17 行响应式调整。分类卡在平板固定为双列，图文块收敛内边距，单海报使用平板专用列比、间距与留白；移动端既有单列规则保持不变。`git diff --cached --check` 与页面装修器区块契约（26 种）通过。

截至此提交，当前工作树中可识别为已引用/已验证方案的前后端源码、数据库 schema/migration 源码、前端工作台、测试、依赖基线、CI 与基础设施配置均已按主题推送。仍未提交的项目资产仅包括真实/结构化环境候选、seed、未知 CSV、Docker override、本机编辑器设置、试运行标记、预览图、以及管理员检查、密码重置和迁移辅助脚本；这些文件未被读取、运行、删除或上传，保留原因见“已排除的批次与原因”。

最终盘点（`549fd56` 推送后）：工作树仍保留 9 个已跟踪与 19 个未跟踪路径，共 28 个用户资产；`git diff --check` 通过，整理分支与 `origin/codex/release-curation-20260814` 无待推送提交。
...
`ac8a3cb` `feat(data): 固化目录与交易领域模型`：按已确认方案提交受控目录、合作商家、交易基础与交易中心共用的 Prisma schema 和三份对应 migration 源码（711 行）。仅版本化源码，不执行 Prisma、migration、seed 或任何数据库连接命令；未包含密码/补偿脚本、未知 CSV、依赖升级或 Docker 覆盖。类型检查与构建证据复用自同一未变更工作树，提交前缓存差异检查通过。
...
## 2026-08-14 发布就绪静态复核（提交后）

### 结论

- 发布整理分支：`codex/release-curation-20260814`；复核前基线为 `5bd0725`，远端分支与本地一致。
- 本次只执行本地、无数据库连接的质量门禁。未读取真实 `.env` 值，未运行 Prisma、migration、seed、Docker、服务进程或部署命令。
- 代码静态质量门禁通过，但尚不构成可直接生产发布的批准：数据库迁移演练、真实环境变量配置、角色权限联调和浏览器运行期冒烟仍是上线前阻断项。

### 验证证据

| 检查 | 结果 | 说明 |
| --- | --- | --- |
| `npm run typecheck` | 通过 | 服务端与客户端 TypeScript 均通过。 |
| `npm run lint` | 通过 | 客户端 ESLint 通过。 |
| `npm run test:selection-inquiry` | 通过 | 20 项选款咨询可见性与安全静态契约通过；真实身份与数据库边界路径尚未联调。 |
| `npm run test:contracts` | 通过 | 页面装修器（Puck）26 种区块契约一致。 |
| `npm run test:trade` | 通过 | 交易状态机 15 项、并发/金额/幂等 18 项、跨层契约 26 项通过；客户公开交易入口仍必须保持冻结。 |
| `npm run build` | 通过 | NestJS 服务端与 Vite 客户端生产构建完成。 |
| `git diff --check` 与 `git diff --cached --check` | 通过 | 未发现空白错误或冲突标记；现有工作树仍有 LF/CRLF 转换提示，未批量改写。 |

### 构建观察与下一阶段

- Vite 报告部分客户端块超过 500 KB：后台编辑器约 714 KB、Ant Design 约 1.15 MB（均为压缩前）。这不是构建失败，但应在运行期验收后安排按路由和重型组件拆分，并用真实网络条件测量首次加载与交互性能。
- 下一阶段必须在隔离 staging 环境进行：使用本分支已提交内容、部署平台注入密钥、数据库负责人先审阅并演练 migration、随后验证公开站、登录/角色权限、选款咨询、Puck 编辑发布、SSE/错误态以及容器健康检查。
- 不得在上述 staging 验证通过前开启交易、订单或支付客户入口；保持 `CUSTOMER_COMMERCE_ENABLED=false`，且不接入真实支付或业务数据。
