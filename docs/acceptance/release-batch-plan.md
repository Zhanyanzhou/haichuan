...
| U17 | `docs/{AUTONOMOUS_EXECUTION_REPORT,PENDING_WORK,PUBLIC_ACCESS_MATRIX,TEMPLATE_CAPABILITY_MATRIX}.md`、`docs/acceptance/{public-runtime-smoke-report,puck-published-content-audit,release-batch-plan}.md`（7） | 必须人工确认业务用途 | 文档是版本化候选，但验收结论、运行报告和 Puck 内容审计须由责任人确认其证据范围与日期；本计划文件可随文档治理批次纳入。 | `PUBLIC_ACCESS_MATRIX`/运行烟测记录可辅助 P0 审阅；Puck 审计明确阻止 Puck 内容混入，但均不解决源文件 hunk。 |
