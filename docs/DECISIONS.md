...
- **机器门禁（2026-08-31 修订）**：项目负责人明确要求模板组件库全部改为通用。`contracts/page-builder/content-templates.contract.json` 的每条 `pageRules.allowedTemplateKeys` 必须完整包含全部 active 模板；动态模板的 `recommendedFor` 只作推荐，不再限制展示、插入或发布。`pageRules` 仍独立约束固定业务区、内容位置和导航模式，视觉细则仍只认 `docs/UI_GUIDE.md`；跨页面通用不得改变页面职责或绕过内容与发布门禁。
...
- **模板与实例**：系统合同继续定义身份、语义角色、槽位数量、默认构图、阅读顺序、行动上限和默认响应式结构；阶段 1 的个人模板保存合同白名单布局与可选默认内容。页面实例覆盖只属于该实例，不得改变模板、其他实例、其他页面或全局设计令牌。旧文档未保存覆盖值时按现有默认渲染，并允许恢复默认。
