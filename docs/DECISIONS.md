...
- **路由职责**：具体页面职责只认 `docs/PROJECT_GUARDRAILS.md`。本决议固定两个产品边界：`/catalog` 是唯一款式搜索、筛选、排序与快速选款中心，但不是唯一购买入口；所有作品可以进入统一详情，并按五种 `SalesMode` 呈现对应公开主行动。合作蜡模报价是审核通过客户在适用设计上的独立能力，不是第六种 `SalesMode`，也不改写作品的公开主行动。
- **表达边界**：品牌叙事页不得退化成商品结果网格、促销会场或价格驱动落地页；可以克制地链接可交易作品详情、顾问咨询或选款中心。商品详情允许真实价格、规格和购买/定制行动，但禁止倒计时、伪折扣、虚假稀缺和强迫式营销。
- **内容信任**：材质、宝石、证书、工艺、产地、库存和价格必须来自可追溯事实；第三方或竞品品牌素材未经可追溯商用授权不得公开。已发布不合规内容通过 Puck/CMS 的草稿、预览、预检、版本发布和公开复核整改，保留历史版本与原文件，不通过删库或覆盖历史消除证据。
...
- **决定源职责**：本条是双客群、三报价、客户确认、合作蜡模价格和订单冻结的产品业务决定源。`PROJECT_RULES.md` 只固化硬门禁，`PRODUCT_DATA_CONTRACT.md` 只定义数据字段与必要校验数值，`DATA_AND_API.md`、`MODULES.md` 只作派生导航与目标边界；发生语义冲突时先回到本条裁决，再由实施阶段核对当前代码事实。
- **经营定位**：公开网站以成熟高级珠宝奢侈品品牌面对普通客户，不强调供应商、批发或工厂直销；合作申请只在“我的账户”内。普通客户以成品珠宝和高级定制为主，审核通过的合作商家获得红蜡/紫蜡 3D 打印蜡模的合作价格与订单能力。
