...
### D.17 ✅ 属性面板成熟化、实例覆盖与页面同类共享设计（2026-08-20；2026-08-28 修订）

- **2026-08-28 修订优先级**：本修订取代下文“画布只预览”“内部对象不进入图层面板”和“禁止跨语义对象叠放”的旧结论。全部 24 个活跃模板进入页面级同类共享设计：同页相同 `moduleType` 的设计实时同步，内容与顶层模块状态独立；主画布是唯一直接布局操作区，小画布只读导航，模板编辑期间图层面板只读展示固定内部对象。固定对象可在模块画框内移动、缩放、重叠和调整层级，但不得新增/删除、移出画框或写任意 HTML/CSS。
- **兼容与发布**：继续使用 `PageDocument` 和 `__instanceOverrides.version=2`，不新增 wire version 或页面级第二事实源。旧草稿与历史 revision 原样读取；同类设计不一致时仅在用户确认进入模板编辑后以当前实例统一，取消零写入。发布阻断结构性不可用状态与同类设计不一致，普通重叠、裁切和文字拥挤只作 warning。
