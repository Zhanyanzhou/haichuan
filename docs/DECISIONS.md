...
- **2026-08-31 统一编辑器双模式决议**：产品采用一套 Shared Editor Core 下的页面装修、模板设计两种工作模式，而不是两套独立编辑器。两种模式尽可能共享 Shell、Template Repository、模板目录、Renderer、Canvas 基础能力、结构树组件、属性面板引擎与字段控件、设备视口、缩放、Selection/Hover、History、Preview 和素材选择；差异集中由 Mode/Capability/Permission 控制选择粒度、命令、结构深度、属性范围、保存对象与业务动作。页面模式以 Template Instance 为主粒度，结构树不展开完整 Node Tree；模板模式开放完整 Node Tree 和合同允许的节点级强编辑。页面与模板会话的 selection、history、dirty 和 save state 必须隔离，但不得据此复制两套编辑器。
- **2026-08-31 页面字段定位决议**：页面 Canvas 命中 Template 明确声明为页面可编辑的图片、文字、行动或业务对象时，可以保持 Template Instance 为主选择，并附加 field/slot focus 让右侧属性面板定位对应字段；命中不可编辑内容时回退总体属性上下文。字段定位不得把内部对象加入页面结构树，不得显示移动/Resize 手柄、触发上传器、执行公开行动或进入内部构图。模板设计负责定义完整属性及“页面可编辑”范围，页面属性面板从同一配置生成或过滤，不得按 Template 硬编码第二套表单。
