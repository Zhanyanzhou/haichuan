...
本节于 2026-09-08 按当前控制器核对。旧 `PageModule` 的 `/published`、`/admin`、`/draft`、`/reorder`、模块级显隐/复制/删除和 `/publish` 端点表已退出本页，不能用于现行接线。

| 对象 | 源码入口 | 路由职责 |
| --- | --- | --- |
| 页面 `PageDocument` | [PageModulesController](../../../server/src/modules/page-modules/page-modules.controller.ts) | `/api/page-modules/document` 下的草稿、预检、发布、公开读取、版本与线上回滚 |
| 母模板 `TemplateDefinitionV2` | [DynamicTemplatesController](../../../server/src/modules/page-modules/dynamic-templates.controller.ts) | `/api/page-modules/dynamic-templates` 下的统一目录、草稿、另存、不可变版本、归档与恢复 |

精确 HTTP 方法、参数、并发条件与角色限制以对应控制器、DTO 和服务为准；本页不维护第二份动作权限表。业务生命周期与源码分工见[装修文档导航](../../page-builder/README.md)。
