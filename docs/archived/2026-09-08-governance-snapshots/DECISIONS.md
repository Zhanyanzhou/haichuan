...
> 本节记录当时固定模板批次的实施理由。模板数量、具体成品和全量完成度已由 D.26 降为当前兼容集合事实，不再定义产品方向或后续模板框架的永久验收目标。
...
> 本节继续约束需要读取旧内容模板合同的兼容路径；新母模板的响应式和比例能力以 `template-definition.schema.json` 及其版本化定义为准，不以本节列出的固定模板和比例清单扩展产品边界。
...
> 本节是内置兼容合同的历史设计决定。框架级双端能力与当前可用节点规则以 `template-definition.schema.json` 为准；是否全验内置模板集合由实际改动是否触及兼容合同或全目录行为决定。
...
- **历史门禁**：修改该内置兼容合同时，生成器继续校验设备范围、顺序覆盖、角色回退和比例白名单；一般共享 Renderer 或框架能力变更按 `WORKFLOW.md` 的实际影响面覆盖节点、槽位、视口和受影响适配器，不再机械绑定固定模板总数。

### D.17 ✅ 统一编辑器双模式、属性面板与模板实例边界（2026-08-20；2026-08-31 修订）

> 本节保留多阶段演进记录。当前产品模型、内容边界与验收只认 D.26 和 `docs/page-builder/template-design-framework.md`；下列被后续条目标注取代的阶段描述不具现行执行力。

- **2026-08-28 阶段 1 历史修订（现行架构以 2026-08-31 统一编辑器双模式决议为准）**：本修订当时取代“页面级同类共享设计”和“页面 Inspector 内模板编辑”的旧结论。编辑器外壳保留四栏，但分为页面、模板两个隔离工作空间：页面 `LayerRail` 永久保持现有页面模块职责，页面属性面板只编辑实例内容和少量允许覆盖；模板设计由管理员从模板组件库进入独立的工具箱、固定对象结构面板、Renderer 画布和模板属性面板。模板会话的选择、设备、撤销、脏状态、预览与保存不得写入 `PageDocument`。
- **兼容与阶段边界**：旧草稿、旧页面、历史 revision 与现有 `__instanceOverrides.version=2` 原样读取、保存和渲染，不自动统一同类实例，也不自动转换个人模板。阶段 1 继续复用当前固定对象合同和个人模板记录格式，保存后立即可用但不称为“发布”；动态节点树、不可变模板版本和正式发布留到后续模型。结构性无效覆盖继续阻断页面发布，普通重叠、裁切和文字拥挤仍只作 warning。
- **2026-08-28 阶段 2 补充决议**：动态模板以 `contracts/page-builder/template-definition.schema.json` 为唯一结构合同，使用稳定 `nodeId` / `slotId`、注册节点类型、嵌套约束、共享内容与桌面/移动独立几何规则；编辑画布、预览和缩略图复用通用 Renderer，客户端与服务端使用同源生成的语义校验器。动态模板当前只允许管理员保存为本机草稿并导入导出，不进入页面、不称为发布，也不改变旧式模板合同或 Renderer。数据库草稿、不可变版本、归档/权限和页面实例须另经阶段 3 审批。
- **2026-08-29 阶段 3 修订（取代上条“仅本机草稿”状态）**：统一 V2 已接入服务端草稿、不可变版本、精确页面实例、稀疏覆盖、复杂节点适配和只读发布影响预检；SYSTEM 模板使用 `ownerId=NULL、sourceType=SYSTEM`，CUSTOM 模板按 `ownerId` 隔离，设计接口仅向 `SUPER_ADMIN` 开放。旧系统/个人模板对外写入口关闭，只保留读取、历史重放和显式转换。直接模板发布、全页面原子激活、Schema/migration、真实数据迁移、旧文件删除与生产切换仍必须分别经过既有高风险门禁，当前实现不得把只读预检表述为已发布或已激活。
- **2026-08-30 原子激活运行门禁决议（已由下条取代）**：当时批准以 `DYNAMIC_TEMPLATE_ACTIVATION_ENABLED` 默认关闭来隔离原子激活；2026-08-31 产品收敛后不再保留可开启的运行能力，只保留历史数据库 ledger 与只读迁移审计资产。
- **2026-08-31 统一母模板决议（取代上方固定/动态并列产品模型及原子激活结论）**：产品只提供一套母模板。模板设计与页面装修消费同一 `TemplateDefinitionV2` 与服务端统一目录；既有系统/个人格式只留在兼容读取与历史重放层，不再作为用户可选择的模板类型。模板设计保存只提供“覆盖模板”和“另存为模板”：覆盖保持同一 `templateId` 并更新草稿，另存创建新的 `templateId`。发布只创建不可变模板版本且不修改任何页面；页面实例锁定精确版本，升级必须在页面装修草稿中显式完成并重新发布页面。Activation 的控制器、服务、客户端和配置开关全部删除；历史 migration、Prisma ledger 模型与 Gate B 纯内存规划器仅为数据库兼容审计保留，不构成可开启的产品能力。本决议不授权 migration、真实数据转换、生产切换或历史表删除。

- **2026-08-31 页面画布交互旧修订（已由下条统一编辑器双模式决议取代）**：当时规定页面画布命中内部内容不得改变属性上下文，实例内容全部从右侧完整属性面板发起；该“不得字段定位”结论不再执行，页面模式仍不开放节点移动、缩放与构图。
- **2026-08-31 统一编辑器双模式决议**：产品采用一套 Shared Editor Core 下的页面装修、模板设计两种工作模式，而不是两套独立编辑器。两种模式尽可能共享 Shell、Template Repository、模板目录、Renderer、Canvas 基础能力、结构树组件、属性面板引擎与字段控件、设备视口、缩放、Selection/Hover、History、Preview 和素材选择；差异集中由 Mode/Capability/Permission 控制选择粒度、命令、结构深度、属性范围、保存对象与业务动作。页面模式以 Template Instance 为结构编辑主粒度，可按需显示已开放内容字段的导航，不因此开放母模板结构写入；模板模式开放完整 Node Tree 和合同允许的节点级编辑。2026-09-08 已取消把“内部树一律不展开”当作硬限制的表述，显示层级与结构写权限分别判断。页面与模板会话的 selection、history、dirty 和 save state 必须隔离，但不得据此复制两套编辑器。
- **2026-08-31 页面字段定位决议**：页面 Canvas 命中 Template 明确声明为页面可编辑的图片、文字、行动或业务对象时，可以保持 Template Instance 为主选择，并附加 field/slot focus 让右侧属性面板定位对应字段；命中不可编辑内容时回退总体属性上下文。字段定位不得把内部对象提升为页面根节点或赋予母模板结构写入权限，不得显示移动/Resize 手柄、触发上传器、执行公开行动或进入内部构图。模板设计负责定义完整属性及“页面可编辑”范围，页面属性面板从同一配置生成或过滤，不得按 Template 硬编码第二套表单。
- **2026-09-02 受控反向提取决议**：页面装修与模板设计继续使用隔离会话和保存对象，但不再把“页面覆盖永远不能进入模板”设为绝对边界。`SUPER_ADMIN` 可从当前 Template Instance 显式执行“应用设计覆盖到母模板草稿”；系统只提取 `TemplateDefinitionV2` 能无损表达的布局/样式字段，以页面锁定的正式版本、当前服务端母模板草稿和页面期望值做三方冲突检测，并在独立模板会话中形成可撤销、尚未保存的修改。文字、图片、视频、链接、商品等真实内容、显隐与业务引用始终排除；偏移、图片缩放、任意百分比焦点等当前合同不能无损表达的字段保留在页面并披露；同字段冲突时整次阻断。该动作不自动保存、发布、升级实例或更新任何页面，模板发布和页面显式升级流程保持解耦。
- **模板与实例**：系统合同继续定义身份、语义角色、槽位数量、默认构图、阅读顺序、行动上限和默认响应式结构；阶段 1 的个人模板白名单布局与可选默认内容只作为旧记录兼容读取，不再是新母模板能力。页面实例覆盖默认只属于该实例，不得隐式改变模板、其他实例、其他页面或全局设计令牌；唯一例外是上条已经批准的受控反向提取。旧文档未保存覆盖值时按原版本渲染，并允许恢复原版本默认。
...
- **现行性**：本节只解释旧 `PersonalContentTemplate.contentDefaults` 的来源和兼容读取，不再授权新母模板保存默认内容。D.26 已决定模板设计只使用系统生成的中性只读示例；新 `TemplateDefinitionV2` 不继续生成、复制或提升 `contentDefaults`、`defaultContent` 或 `previewContent`。
- **历史行为**：当时账号私有模板默认只保存布局与受控视觉属性；只有运营人员显式选择“同时保存当前默认内容”才写入可空 `contentDefaults`。现行兼容读取仍按原记录和原版本解释，不静默删除、迁移或覆盖。
...
### D.26 ✅ 模板设计框架优先与内置模板重新定位（2026-09-03）

- **产品方向**：店铺装修的核心是可持续扩展的模板设计框架，不是预先维护固定数量的成品模板。框架应优先完善结构、槽位、构图、样式、响应式、实例可编辑范围、版本与同源渲染能力。
- **内置模板定位**：当前 24 个内置模板继续用于历史兼容和快速起步，但其数量、具体内容和完成度不构成产品边界、固定路线图或所有模板任务的验收总目标。新增母模板不以加入内置模板清单为前提。
