...
### 页面装修与公开路由源码（历次记录；2026-09-08 局部复核）

> 本次仅核对合同版本、桌面布局源码、空草稿预览和服务端发布校验；下列旧日期的测试与环境结论未重跑。实现入口见[页面边界](../../architecture/page-builder-boundary.md)，历史报告见[文档导航](../../page-builder/README.md)，功能目标见[专项规格](../../page-builder/template-design-framework.md)。

- Puck `PageDocument` 是唯一活跃装修体系，贯穿页面模块后端、编辑器、预览、公开 Renderer、区块组件和适配器。
- 2026-09-08 源合同复核：`contracts/page-builder/content-templates.contract.json` 的 `contractSchemaVersion=9`、`registryVersion=18`、`publicationGateVersion=3`；内置登记/活跃数量均为 24，`pageRules` 为 6 条。本条只描述内置兼容集合的源码快照；结构合同、实际 Repository 与页面实例的职责见[专项框架](../../page-builder/template-design-framework.md)第 2 节。
...
- 旧共享界面与桌面收起行为的测试只适用于原基线；本节不继续复制旧坐标、40px 收起尺寸或旧控件清单。历史环境和证据层见 [2026-08-30 闭环审计](../2026-09-08-page-builder-evidence/STORE_DECORATION_CLOSURE_AUDIT_2026-08-30.md)。
- 2026-09-08 桌面布局源码复核：`editor.css` 已按 `PROJECT_RULES.md` §7 固定四区，桌面收起按钮隐藏，收起状态不改变列宽；本轮没有执行浏览器视口测量。
...
- 模板目录与设计画布已使用系统生成的只读示例和同源 Renderer，不保存独立缩略图；槽位语义边框只在选中、Hover 或键盘聚焦时显示。模板属性不提供真实正文、媒体上传、业务对象、跳转或默认内容编辑。新母模板内容为空，旧 `defaultContent/previewContent` 只兼容读取；`emptyPolicy=use-default` 只读精确历史版本默认值，新身份或显式清理归一为 `hide`。复杂空态按嵌套内容判空，复杂节点设计写入 `contentTemplateDesignProps`；页面保持实例选择，内部节点几何操作留在母模板工作面。
...
- 草稿预览、线上只读比较和失败恢复已接入：无草稿显示安全空态，读取失败可重试；查看线上版本保留完整编辑快照，返回时复用基线；比较和历史恢复先排空在途保存。各动作保护边界集中见[实现边界](../../architecture/page-builder-boundary.md)第 5 节，本节不再复制整套交互要求。
- **2026-09-08 发布校验源码复核**：就绪度来自服务端 `PageModulesService.collectPageDocumentValidation`。当前 metadata 校验将 SEO/OG、内容负责人缺项记为 `error`，素材授权校验要求可见素材逐 URL 提供记录；这些错误会阻断发布。此前本节“只作可选建议、不阻断发布”的描述不符合当前服务端。`PageSettingsDrawer` 仍有“素材记录可选、不影响发布”的提示，属于界面与服务端的已知差异。本轮只纠正文档，未更改发布行为、裁定未来产品取舍或验证真实发布。
...
- JSON 方案导入与草稿加载、历史恢复共用 `ensureEditorPageStructure`，按当前页面归一化根内容并披露移除项；导入只写内存并保留未保存保护。导入细则见[实现边界](../../architecture/page-builder-boundary.md)第 6 节。旧发布快照的 zones 只在读取副本中清空；旧验收印记须重新复核发布。
...
- 中性示例实现：内置起步/兼容模板示例媒体使用系统中性 SVG，经同源 Renderer 展示真实结构，不加载真实摄影栅格图；示例数据边界只维护在[专项框架](../../page-builder/template-design-framework.md)第 4 节。
...
### 模板编辑能力与待验项（2026-09-08 源码核对）

已删除被替代的近期执行计划；本节保留有效证据与回归线索，产品目标统一见[功能规格](../../page-builder/template-design-framework.md)。以下未做本轮浏览器、保存重载或真实 API 验收。

| 判断 | 当前证据与下一项验证 |
| --- | --- |
| 起步骨架已有源码 | [TemplateWorkspaceController](../../../client/src/page-builder/template-editor/TemplateWorkspaceController.tsx) 的 `dynamic-new` 调用 [createNewDynamicTemplateDraft](../../../client/src/page-builder/template-editor/dynamicTemplateDraftRepository.ts)，已构造区域与图片/标题/正文；先验首次进入、删除/添加和保存重开，不再按旧计划重做 |
| 连续添加与合法插入已有源码 | [DynamicTemplateToolbox](../../../client/src/page-builder/template-editor/DynamicTemplateToolbox.tsx) 成功后保持添加入口打开；父级判断复用工具函数和 operations。待验连续添加三项、非法移动、Esc 与焦点恢复 |
| 图片与文字字段已有源码 | [TemplateSlotControls](../../../client/src/page-builder/template-editor/TemplateSlotControls.tsx) 已有比例、适配、焦点和文字规则；转为分组可理解性、输入边界、双端渲染与保存重载验证，不再重复“补齐字段” |
| 简单槽位的来源与恢复待完善 | [Inspector](../../../client/src/page-builder/template-editor/DynamicTemplateInspectorPanel.tsx) 的 `simpleSlot` 路径跳过完整布局面板，未获得同等双端来源/复制入口，简单控件无明确单字段恢复。目标中的“恢复最近保存值”尚需接线与验证，不可冒充既有能力 |
| 流式槽位缺少画布直接调整入口 | [Canvas](../../../client/src/page-builder/template-editor/DynamicTemplateCanvas.tsx) overlay 只覆盖自由 placement 与内置合同角色；普通流式槽位可用面板调尺寸。待按流式重排语义接线，不应把“无手柄”描述成“不能调尺寸” |
| 系统示例压力状态未核验 | 短/长文、媒体比例和缺失内容切换是功能规格中的设计目标；本轮未确认当前入口或 Renderer 已支持，不能当作完成事实 |

仍需复现的历史线索：系统修复方案曾被当成用户修改，取消后反复进入修复；目录曾出现“模板模板”重复命名；中性示例曾触发真实图片清晰度警告。使用专用测试草稿复验，不能继承旧计划“直接用真实首屏”的操作建议。9 月 6 日目录刷新与版本插入问题按[原验收报告](../2026-09-08-page-builder-evidence/DYNAMIC_TEMPLATE_FIVE_REAL_ACCEPTANCE_2026-09-06.md)定向回归，未复现前不计为当前缺陷。
