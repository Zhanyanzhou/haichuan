...
> **⚠️ 历史证据边界**：本文件记录特定日期、视口、页面状态与当时实现的检查结果，仅用于追溯。竞品、用户参考图和历史对比样本不得作为复刻依据，也不是海川当前规范或现行验收权威；新任务必须以当前 `docs/PROJECT_GUARDRAILS.md`、`docs/UI_GUIDE.md`、真实页面与当次验证为准。以下历史证据正文保持原样，不代表当前页面仍已通过。
...
## 2026-09-06：模板设计“简单模板 / 无折叠”历史验收

> 本节由根目录重复的 `design-qa.md` 收敛而来，只保留当时本地 Edge 验收的产品要求与证据边界，不代表 2026-09-08 当前工作树已复验。

**当时确认的交互边界**

- 模板设计只定义尺寸和图片、标题、正文、按钮槽位；真实内容在页面装修中填写。
- 模板库使用平铺筛选和连续卡片列表；结构、区域、槽位及属性保持持续可见，不使用折叠或手风琴。
- 复制、排序、隐藏、锁定和删除统一归入槽位操作；设备切换只保留顶部一处，宽高固定显示在画布上方。
- 添加入口只提供四种常用槽位并自动建立或复用内容区域；普通区域直接设置排列、对齐、间距与留白。
- 旧兼容模板保持原生规则，不批量迁移旧定义；图片比例与高度模式同步，根节点锁定后禁止修改设计宽度。
- 删除区域必须检查必填后代；新建的普通可选槽位仍可删除和撤销。

**当时的本地证据与限制**

- 独立 Edge 在 1920×960 与 1600×1000 视口、确定性 API 夹具下覆盖新建、保存、刷新回读、测试版本发布、页面填充、锁定/解锁、删除/撤销和旧尺寸兼容。
- 目标 TypeScript 与 ESLint 检查通过；可按“简单模板 Edge 验收 / 简单模板保护 / 方案二预设兼容清理”三个历史测试标题定位当时用例。
- 没有保存、覆盖或发布真实模板/页面，没有执行数据库 migration，也没有验证生产发布或公开网站消费；当时仍记录既有 ResizeObserver 与 Puck `setData` 警告。
